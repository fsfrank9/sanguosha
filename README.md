...
- **内容**:71 名武将 / 53 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经四轮全量合规审计。
- **AI**:整回合深度模拟 + 可见信息诚实计数(不读暗牌)+ 多人目标评估 + 暗身份行为推断(不读暗置身份,守护测试常驻);对 v11 冻结基线胜率 63.0%(200 局固定种子,≥55% 门禁 `tests/v12_i_benchmark` 常驻),暗身份推断准确率 5p 56% / 4p 48% 对随机基线 37.5%/33.3%(门禁 `tests/v13_m4_inference_benchmark` 常驻;数字随批次演进重测,当前为 2026-07-31 实跑值)。
...
- **下一步 `v14`(规划)**:工程清账 → 多目标杀架构(方天画戟/流离销账)→ AI 博弈深化(无懈 EV 补齐/内奸带门槛)→ 内容与模式决策门(蛊惑版本裁定/KOF 评估);6 人以上大场经用户裁定收案不做(5 人封顶)。见 [v14 路线图](docs/plans/2026-07-31-sanguosha-v14-roadmap.md)。
...
npm run verify        # build:check + 全量测试(CI 门禁同款, ~50s)
```
...
docs/
  history.md          版本演进史(v4 → v13 的逐版档案;尾部批次补档排入 v14-O3)
  plans/              各版本计划与执行记录
...
- 多人专属技(激将/护驾/黄天/离间)随身份场(3/4/5 人)激活,1v1 中保持惰性;蛊惑留待多人评估。
...
| v14(规划) | 多目标杀架构 + AI 博弈深化 + 工程清账 | `2026-07-31-…-v14-roadmap.md` |
