...
- **内容**:71 名武将 / 53 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经三轮全量合规审计。
- **AI**:整回合深度模拟 + 可见信息诚实计数(不读暗牌)+ 多人目标评估 + 暗身份行为推断(不读暗置身份,守护测试常驻);对 v11 冻结基线胜率 62.5%(200 局固定种子,≥55% 门禁 `tests/v12_i_benchmark` 常驻),暗身份推断准确率 5p 55% / 4p 48% 对随机基线 37.5%/33.3%(门禁 `tests/v13_m4_inference_benchmark` 常驻;数字随 v13-N 牌堆扩容重测)。
- **当前版本 `v13-N`**:内容评估批收官 — 军争名录零缺口确认、四武器(寒冰/古锭/朱雀/银月)入牌堆(142 张全可达)、奸雄×多目标守恒修复;蛊惑评估记录为立项候选。v13 路线图 J-N 全部完成,见 [v13 路线图](docs/plans/2026-07-16-sanguosha-v13-roadmap.md)。
...
docs/
  history.md          版本演进史(v4 → v13-N 的逐版档案)
  plans/              各版本计划与执行记录
  audit/              合规审计纪要(三轮)
```
