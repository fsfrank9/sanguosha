...
- **内容**:71 名武将 / 67 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经四轮全量合规审计;蛊惑(于吉,风包现行版)带质疑链与「缠怨」,使用流程 16 型 + 响应窗口打出流程八窗口全接入(v14 R1 + v15 S)。
...
- 武将 71 名 / 技能条目 128 条 / 唯一技能 ID 123 个;已接入引擎 67 个(含主动/交互 16 个、风包 10 技与火包 13 技),未实现技能在 UI 中明确标记,不会"看起来有但触发不了"。
