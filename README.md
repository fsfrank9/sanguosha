...
- **内容**:79 名武将 / 125 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经六轮全量合规审计;蛊惑(于吉,风包现行版)带质疑链与「缠怨」,使用流程 16 型 + 响应窗口打出流程八窗口全接入(v14 R1 + v15 S)。
- **AI**:整回合深度模拟 + 可见信息诚实计数(不读暗牌)+ 多人目标评估 + 暗身份行为推断(不读暗置身份,守护测试常驻)+ 无懈全类期望值 + 内奸装忠/拆家/收割序博弈(v14 Q,量化门禁 `tests/v14_q2_renegade_benchmark` 常驻);对 v11 冻结基线胜率 64.0%(200 局固定种子,≥55% 门禁 `tests/v12_i_benchmark` 常驻),暗身份推断准确率 5p 67.9% / 4p 69.1% 对随机基线 37.5%/33.3%(门禁 `tests/v13_m4_inference_benchmark` 常驻;数字随批次演进重测,当前为 2026-09-08 实跑值)。
...
- **上一版 `v15`(已收官)**:五个阶段全部交付 —— **S** 蛊惑收尾(响应窗口打出流程八窗口、声明菜单 16/16、AI 全型声明启发与五面质疑,spec 缺口 14 条逐条裁定[简报](docs/audit/2026-08-05-guhuo-spec-gaps.md));**T** 火包 8 将 13 技 + 全新拼点框架([简报](docs/audit/2026-08-05-fire-pack-spec.md));**U** 林包 8 将 18 技([简报](docs/audit/2026-08-06-lin-pack-spec.md));**V** 山包 7 将 15 技 + 觉醒技骨架/"田"区/失牌时机/额外回合机制,左慈化身经独立成本评估门推迟([简报](docs/audit/2026-08-06-shan-pack-spec.md));**W** 清账收官 —— 散账六项二态归零 + 第五轮全量合规审计(10 域并行 · 27 agent · 16 候选 → 确证 11 · 驳倒 5,其中两条驳回打在本轮自己的修复上并已还原)+ 三条长期不变量与守恒 soak 固化入库([账本](docs/audit/2026-08-06-w-ledger.md));收官后 **X** 批把审计留账 F8-F14 一次性清偿(对抗复现 6 确证 / 1 驳倒已修,[明细](docs/audit/2026-08-07-x-ledger-clear.md))。见 [v15 路线图](docs/plans/2026-08-04-sanguosha-v15-roadmap.md)。
- **当前版本 `v16`(Y–AC 已收官)**:**Y** 五条响应链统一挂起续跑、AI 蛊惑响应八窗口与四项留账修复([验收记录](docs/audit/2026-09-07-v16-y-response-protocol.md));**Z** 单骑/庸肆/伪帝 3 技、巧变跳过出牌与跨区移牌、AI 保桃、拼点失牌时机及挂起清算([验收记录](docs/audit/2026-09-07-v16-z-completion.md));木牛流马按 Z-G1 默认边界不加入。**AA** 动态技能三态层、30处有效属性读取、场外武将牌池与回合边界选择、持续效果修复已交付；左慈两技按 AA-G1 基建后实测继续推迟([验收记录](docs/audit/2026-09-08-v16-aa-completion.md))。**AB** 按用户点单新增八神将、20原生技能与派生极略，含势力选择、星牌区、多牌龙魂、可暂停判定与技能结算([验收记录](docs/audit/2026-09-08-v16-ab-completion.md))。**AC** 完成第六轮十域审计、六项常驻不变量、跨包AI实测及修复，被动弃牌选择与遗留决策逐项结案([验收记录](docs/audit/2026-09-08-v16-ac-completion.md))。见 [v16 路线图](docs/plans/2026-08-07-sanguosha-v16-roadmap.md)。

当前发布结果见 [v16 最终收尾](docs/audit/2026-09-08-v16-release-closeout.md)。下一版已形成 [v17 路线图](docs/plans/2026-09-08-sanguosha-v17-roadmap.md)，**实施尚未开始，下一阶段为 AD：同一时机触发顺序与可恢复调度**。
...
- 武将 79 名 / 技能条目 149 条 / 唯一挂将技能 ID 144 个;已接入引擎 125 个(含神包20原生技能，以及派生技能「急袭」「极略」),挂将未接入唯一技能 21 个,未实现技能在 UI 中明确标记,不会"看起来有但触发不了"。
- 多人专属技(激将/护驾/黄天/离间)随身份场(3/4/5 人)激活,1v1 中保持惰性;蛊惑(风包现行版)已全流程接入 — 出牌阶段使用流程 16 型声明 + 响应窗口打出流程(闪应杀/万箭/银月、杀应决斗/南蛮/借刀、桃·酒应濒死、无懈应锦囊,共八个窗口)+ 全场质疑链 + 缠怨,1v1 与身份场均可用;v16 Y 已接入 AI 席响应声明,原生响应优先、验假后自动回到原响应流程。
...
| v15-X | 审计留账 F8-F14 一次性清偿(对抗复现 6 确证 / 1 驳倒已修):借刀转化面同点收口、猛进/贯石斧武将技先行双挂起链、闪电×帷幕、银月枪"使用"半面带终局/濒死守卫、妄尊时序、铁骑先于雌雄、AOE 无懈展示目标 | `docs/audit/2026-08-07-x-ledger-clear.md` |
| v16-Y | 五链统一挂起 + AI 蛊惑响应 + 四项留账修复;通用钩子链早退显式滚动 AC4 | `docs/audit/2026-09-07-v16-y-response-protocol.md` |
| v16-Z | SP 3 技 + 巧变出牌阶段分支 + AI 保桃/拼点清算;木牛流马默认边界不加入 | `docs/audit/2026-09-07-v16-z-completion.md` |
| v16-AA | 动态技能与有效属性单点、场外武将牌和可挂起边界；化身/新生按新成本门继续推迟 | `docs/audit/2026-09-08-v16-aa-completion.md` |
| v16-AB | 八神将独立目录、21技能、势力选择、星牌区与可暂停结算 | `docs/audit/2026-09-08-v16-ab-completion.md` |
| v16-AC | 第六轮十域审计、六项常驻不变量、跨包AI实测与修复、被动弃牌选择、遗留决策收官 | `docs/audit/2026-09-08-v16-ac-completion.md` |
| v16-发布收尾 | #200／#201已合并，最终251文件全绿，Pages及线上验收通过，旧工作分支已清理 | [最终收尾](docs/audit/2026-09-08-v16-release-closeout.md) |
| v17（规划） | AD共享时机调度 → AE移动事实／装备时机 → AF既有交互 → AG化身双门 → AH审计；尚未实施 | [v17路线图](docs/plans/2026-09-08-sanguosha-v17-roadmap.md) |

## 官方资料对照与缓存

官网资料分两层,避免重复拉取、也避免在公开仓库提交大段官网原文:

- `tests/fixtures/official_*_skills.json`:官网武将/技能名紧凑 fixture(标准包恒 27 将;风包 8 将 12 技（含奋激TODO）,含 `implementationStatus` 如实标记,gid 为临时编号、核对进度见文件内 `gidPolicy`)。
