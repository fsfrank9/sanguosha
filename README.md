...
- **当前版本 `v16`(规划)**:四主线五阶段草案 —— **Y** 响应链可挂起化(全仓唯一 P0"双闪同步循环无停机"+ AI 席蛊惑声明解锁 + 低项留账清零);**Z** 逐字源残余回收(SP 单骑/庸肆/伪帝 3 技 + 巧变"跳过出牌阶段"缺口,内容面就此见底);**AA** 动态技能三态层 + 性别/势力覆写单点(158 处 `hasSkill` 收口)+ 左慈二次成本门;**AB** 目录边界决策门(神武将 8 将预案,默认不开、需点单);**AC** 收官批(第六轮全量审计 + 不变量 3→≥5 条 + AI 跨包启发回归)。见 [v16 路线图](docs/plans/2026-08-07-sanguosha-v16-roadmap.md)。
...
| v16(规划) | Y 响应链可挂起化 / Z 逐字源残余回收 / AA 动态技能层+左慈门 / AB 目录边界门(默认不开) / AC 收官审计,四道决策门前置定价 | `2026-08-07-…-v16-roadmap.md` |
