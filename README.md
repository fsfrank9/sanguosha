...
**当前版本 `v12-H 多人身份场`**:双模式并存——1v1 之上落地 3 人身份场(主公/反贼/忠臣)。座次环结算(无懈按座次队列逐位询问并逐张重建、南蛮/万箭逐席结算含濒死挂起续跑、决斗跨座、铁索传导环、闪电移位环)、全牌类显式目标(`options.target` + 座席级合法目标矩阵;identity3 恢复顺手/兵粮官方距离≤1,1v1 保持变体)、身份死亡结算(阵亡弃置所有牌、击杀反贼摸三张、主公误杀忠臣弃牌、回合中阵亡即终止回合)与主/反胜负判定。**53 个已实现技能**(11 个主动)和 71 名武将:H7 激活主公技激将/护驾(同势力代打杀/闪,玩家代打者挂起询问)、黄天(群势力给牌)与离间(虚拟决斗);AI 具备阵营立场(无懈立场门、忠臣救主、缺省目标只落敌对座席)。承接 F 域模块架构,牌守恒 census 覆盖任意座席数。**1v1 红线:全量既有测试零改动通过。**
...
3. **H 多人模式:引擎侧收官** — H1-H5/H7 全部落地:座次环结算(无懈队列/AOE逐席/决斗跨座/铁索环/闪电环/桃园五谷座次)、全牌类显式目标 + 座席级合法目标矩阵、identity3 距离规则(顺手/兵粮≤1)、身份死亡结算(弃置所有牌+奖惩+回合终止)、主公技激活(激将/护驾/黄天)+ 离间(技能 49→53,跨 G"≥50"验收线)、AI 阵营立场;H6 多座 UI 见对应批次。全程 1v1 行为零回归(既有测试零改动)。
4. **I AI 进阶:未开始**(I3 多人目标评估已具备 H 基础)。
