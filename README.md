...
9. Phase 4D 已把周瑜【英姿】迁入 `onDrawPhase` hook：摸牌阶段统一派发 draw-phase hook，默认摸 2、【英姿】额外摸 1、【突袭】偷牌并少摸 1 的既有行为保持不变。
10. v4 继续保证根目录 `index.html` 与 `dist/index.html` 可直接 `file://` 打开且字节级一致；v5 方向则是 GitHub 托管访问、模块化加载，不再维护 all-in-one 单 HTML 作为架构目标。
...
- 【英姿】：周瑜摸牌阶段额外摸 1 张牌。该触发入口已迁到 Phase 4D 的 `onDrawPhase` hook seam，`performDrawPhase` 只负责派发 draw-phase hook 与执行摸牌，【突袭】的偷牌/少摸 1 张逻辑保持原行为。
