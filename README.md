...
- **内容**:71 名武将 / 54 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经四轮全量合规审计;蛊惑(于吉,风包现行版)带质疑链与「缠怨」,使用流程 16 型 + 响应窗口打出流程六窗口全接入(v14 R1 + v15 S)。
...
- **当前版本 `v13-收官`**:路线图 J-N 全部完成后,又以九个实测驱动批收尾 — UI 修缮一~三批、武将图鉴及续批、第四轮全量审计 16 条修复、张角修缮一~三(雷击/鬼道语义与时机、面板花色可读、延时锦囊无懈询问),见 [v13 路线图](docs/plans/2026-07-16-sanguosha-v13-roadmap.md) 尾部执行记录。
...
- **当前版本 `v15`(进行中)**:主线 = 三扩展包内容接入(火 13 技 / 林 18 技 / 山 17 技 — 收官盘点证实 48 技逐字官方文本已全在本地 gltjk 镜像,纠正 v13 N3 "无 spec 源" 误判)+ 蛊惑收尾 + 清账收官(散账显式裁决 + 第五轮全量审计);左慈化身设独立成本评估门。**S 蛊惑收尾已交付**:响应窗口打出流程六窗口、声明菜单 16/16、AI 全型声明启发与五面质疑,spec 缺口 12 条逐条裁定入库([简报](docs/audit/2026-08-05-guhuo-spec-gaps.md))。见 [v15 路线图](docs/plans/2026-08-04-sanguosha-v15-roadmap.md)。
...
docs/
  ...
  audit/              合规审计纪要(第一/三/四轮;第二轮档案见 history.md)
                      + 官方文本 spec 缺口裁定简报(如蛊惑 12 条)
```

## 内容现状

- 武将 71 名 / 技能条目 128 条 / 唯一技能 ID 123 个;已接入引擎 54 个(含主动/交互 12 个与风包 10 技),未实现技能在 UI 中明确标记,不会"看起来有但触发不了"。
- 多人专属技(激将/护驾/黄天/离间)随身份场(3/4/5 人)激活,1v1 中保持惰性;蛊惑(风包现行版)已全流程接入 — 出牌阶段使用流程 16 型声明 + 响应窗口打出流程(闪应杀/万箭/银月、杀应决斗、桃·酒应濒死、无懈应锦囊)+ 全场质疑链 + 缠怨,1v1 与身份场均可用;AI 席在响应窗口不声明蛊惑为已知局限(见 v15 路线图 S 记录)。
...
| v15-T/U/V/W(规划) | 三扩展包接入(火/林/山 48 技)+ 清账收官 | `2026-08-04-…-v15-roadmap.md` |
