...
- **当前版本 `v16`(Y/Z 已交付)**:**Y** 五条响应链统一挂起续跑、AI 蛊惑响应八窗口与四项留账修复([验收记录](docs/audit/2026-09-07-v16-y-response-protocol.md));**Z** 单骑/庸肆/伪帝 3 技、巧变跳过出牌与跨区移牌、AI 保桃、拼点失牌时机及挂起清算([验收记录](docs/audit/2026-09-07-v16-z-completion.md));木牛流马按 Z-G1 默认边界不加入。后续规划:**AA** 动态技能三态层 + 性别/势力覆写单点 + 左慈二次成本门;**AB** 目录边界决策门(默认不开、需点单);**AC** 收官审计与遗留局限裁定(含 Y5 通用钩子链早退)。见 [v16 路线图](docs/plans/2026-08-07-sanguosha-v16-roadmap.md)。
...
| v16(后续规划) | AA 动态技能层+左慈门 / AB 目录边界门(默认不开) / AC 收官审计 | `2026-08-07-…-v16-roadmap.md` |
