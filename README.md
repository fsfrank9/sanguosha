...
**当前版本 `v12-G`**:在 v12-F 结构减重基础上完成扩展包技能池阶段。风包/扩展包 10 个 1v1 可达技能接入，技能注册、判定区结算、playCard 分发入口、大厅/选将 UI 面板保持模块化;50 个已实现技能(10 个主动)和 71 名武将行为保持 1v1 零回归。牌移动统一走 `moveCard` 原语并由全局牌数守恒断言护航;AI 具备转化 lookahead 与无懈/闪响应期望值决策。
...
路线图见 [`docs/plans/2026-07-05-sanguosha-v12-roadmap.md`](docs/plans/2026-07-05-sanguosha-v12-roadmap.md),当前 **G 扩展包技能池已完成**;后续按依赖排序继续推进:

1. **H 多人模式**(主战役):座次抽象、距离环、目标选择框架、3 人身份场;红线 1v1 零回归。
2. **I AI 进阶**:两步 lookahead、可见信息计数建模、多人目标评估。
