...
- **内容**:71 名武将 / 85 个已接入技能 / 标准+军争核心 39 个牌型 142 张全可达,全部数据驱动;结算对照 `official-skill-cache/` 官方规则集,历经四轮全量合规审计;蛊惑(于吉,风包现行版)带质疑链与「缠怨」,使用流程 16 型 + 响应窗口打出流程八窗口全接入(v14 R1 + v15 S)。
...
- **当前版本 `v15`(进行中)**:主线 = 三扩展包内容接入(火 13 技 / 林 18 技 / 山 17 技 — 收官盘点证实 48 技逐字官方文本已全在本地 gltjk 镜像,纠正 v13 N3 "无 spec 源" 误判)+ 蛊惑收尾 + 清账收官(散账显式裁决 + 第五轮全量审计);左慈化身设独立成本评估门。**S 蛊惑收尾已交付**:响应窗口打出流程八窗口、声明菜单 16/16、AI 全型声明启发与五面质疑,spec 缺口 14 条逐条裁定入库([简报](docs/audit/2026-08-05-guhuo-spec-gaps.md))。**T 火包已交付**:8 将 13 技全部接入 + 全新拼点框架([简报](docs/audit/2026-08-05-fire-pack-spec.md))。见 [v15 路线图](docs/plans/2026-08-04-sanguosha-v15-roadmap.md)。
...
- 武将 71 名 / 技能条目 128 条 / 唯一技能 ID 123 个;已接入引擎 85 个(含主动/交互 18 个、风包 10 技、火包 13 技与林包 18 技),未实现技能在 UI 中明确标记,不会"看起来有但触发不了"。
...
| v15-U/V/W(规划) | 林/山两包接入(35 技)+ 清账收官 | `2026-08-04-…-v15-roadmap.md` |
