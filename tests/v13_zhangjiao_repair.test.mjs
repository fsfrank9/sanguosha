...
test('R6: 无效目标 → fail 且重挂, 重选合法目标可继续 (选择不丢失)', () => {
  ...
  assert.equal(game.pendingChoice && game.pendingChoice.kind, 'leiji-ask', 'fail 后重挂 (本文件重挂惯例)');
  const enemyHpBefore = game.enemy.hp;
...
// ───── 评审收口: 双改判退让 + AOE 闪化解挂起守卫 ─────────────────────

function build3p(opts) {
  opts = opts || {};
  const seats = ['player', 'enemy', 'ally'];
  const game = Engine.newGame({
    seed: opts.seed || 26101,
    seats,
    playerHero: opts.playerHero || 'simayi',
    enemyHero: opts.enemyHero || 'zhangjiao',
    allyHero: opts.allyHero || 'liubei'
  });
  game.log = [];
  game.discard = [];
  game.deck = [];
...
test('R16: 玩家鬼才询问挂起时 AI 鬼道彻底退让 — 不再在面板背后偷换判定牌', () => {
  // 对抗验证实证场景: AI 张角雷击判红桃 → 玩家司马懿鬼才 ask 挂起;
  // 修复前 AI 鬼道 auto 仍把红桃换成黑桃 (面板展示陈旧, 玩家据"无害
  // 红桃"放弃改判 → 被隐藏黑桃命中吃 2 点雷伤)。修复后后到者退让。
  const game = build3p();
  game.player.hand = [c('sha', { id: 'p-sha' }), c('tao', { id: 'gc-heart', suit: 'heart', color: 'red' })];
  game.enemy.hand = [c('shan', { id: 'zj-shan' }), c('sha', { id: 'zj-spade', suit: 'spade', color: 'black', rank: '3' })];
  game.deck.push(c('sha', { id: 'leiji-judge', suit: 'heart', color: 'red', rank: '4' }));
  const playerHpBefore = game.player.hp;
  const r = Engine.playCard(game, 'player', 'p-sha', { target: 'enemy' });
  assert.equal(r.ok, true, r.message);
  assert.equal(game.pendingChoice && game.pendingChoice.kind, 'guicai-replace', 'AI 雷击判定挂玩家鬼才询问');
  assert.equal(game.pendingChoice.judgementCard.suit, 'heart', '面板展示的原判定牌为红桃');
  assert.ok(!game.log.some((l) => l.includes('【鬼道】')), 'AI 鬼道退让, 未在面板背后换牌');
  const declineResult = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: null }));
  assert.equal(declineResult.ok, true, declineResult.message);
  assert.equal(game.player.hp, playerHpBefore, '原红桃判定生效 → 未中, 玩家所见即所得');
  assert.ok(game.enemy.hand.some((x) => x.id === 'zj-spade'), '张角黑桃留手 (未被 auto 偷换)');
  assert.equal(game.pauseState.leiji, null, '雷击挂起快照清空');
});

test('R17: 万箭闪化解触发 leiji-ask → AOE 循环挂起不越过, 排空后续跑剩余座席', () => {
  // 评审收口: aoeEffectForCurrent 闪化解路径此前缺 pendingChoice 守卫,
  // 雷击挂起会被越过先结算后续座席。
  const game = build3p({ playerHero: 'zhangjiao', enemyHero: 'liubei', allyHero: 'huangzhong' });
  game.turn = 'ally';
  game.ally.hand = [c('wanjian', { id: 'a-wj' })];
  game.player.hand = [c('shan', { id: 'p-shan' })];
  game.enemy.hand = []; // 无闪, 轮到即吃 1 伤
  game.deck.push(c('sha', { id: 'leiji-judge', suit: 'spade', rank: '4' }));
  const enemyHpBefore = game.enemy.hp;
  const r = Engine.playCard(game, 'ally', 'a-wj');
  assert.equal(r.ok, true, r.message);
  assert.equal(game.pendingChoice && game.pendingChoice.kind, 'leiji-ask', '玩家闪化解后挂雷击询问');
  assert.equal(game.enemy.hp, enemyHpBefore, '挂起期间未越过结算后续座席');
  assert.ok(game.pauseState.aoe, 'AOE 队列保留待续跑');
  const declineResult = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { decline: true }));
  assert.equal(declineResult.ok, true, declineResult.message);
  assert.equal(game.enemy.hp, enemyHpBefore - 1, '选择排空后 AOE 续跑, 剩余座席正常结算');
...
// ───── 张角三修: 鬼道"替换"=获得原判定牌 + 延时锦囊无使用者无懈 ─────────

test('R21: 鬼道用手牌替换 → 张角获得原判定牌进手牌 (替换非代替), 守恒', () => {
  const game = buildGame();
  game.player.judgeArea = [c('lebusishu', { id: 'r21-le' })];
  game.player.hand = [c('sha', { id: 'r21-black', suit: 'club', color: 'black', rank: '9' })];
  for (let i = 0; i < 6; i += 1) game.deck.push(c('sha', { id: 'r21-pad-' + i }));
  game.deck.push(c('tao', { id: 'r21-orig', suit: 'diamond', color: 'red', rank: '3' }));
  Engine.startTurn(game, 'player');
  assert.equal(game.pendingChoice && game.pendingChoice.kind, 'guidao-replace');
  const r = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'r21-black' }));
  assert.equal(r.ok, true, r.message);
  assert.ok(game.player.hand.some((x) => x.id === 'r21-orig'), '原判定牌 → 张角手牌 (获得)');
  assert.ok(!game.discard.some((x) => x.id === 'r21-orig'), '原判定牌不进弃牌堆');
  assert.ok(game.discard.some((x) => x.id === 'r21-black'), '替换牌结算后进弃牌堆');
  assert.ok(game.log.some((l) => l.includes('获得原判定牌')));
});

test('R22: 鬼道用装备牌替换 → 装备离场进弃牌堆, 原判定牌进手牌', () => {
  const game = buildGame();
  game.player.judgeArea = [c('lebusishu', { id: 'r22-le' })];
  game.player.equipment.weapon = c('qinggang', { id: 'r22-eq', suit: 'spade', color: 'black' });
  for (let i = 0; i < 6; i += 1) game.deck.push(c('sha', { id: 'r22-pad-' + i }));
  game.deck.push(c('tao', { id: 'r22-orig', suit: 'diamond', color: 'red', rank: '3' }));
  Engine.startTurn(game, 'player');
  const r = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'r22-eq' }));
  assert.equal(r.ok, true, r.message);
  assert.equal(game.player.equipment.weapon, null, '装备离场');
  assert.ok(game.discard.some((x) => x.id === 'r22-eq'), '装备替换牌进弃牌堆');
  assert.ok(game.player.hand.some((x) => x.id === 'r22-orig'), '原判定牌 → 张角手牌 (获得)');
});

test('R23: 对照 — 鬼才 (司马懿) "代替" 仍弃置原判定牌 (不获得)', () => {
  const game = buildGame({ playerHero: 'simayi', enemyHero: 'liubei' });
  game.player.judgeArea = [c('lebusishu', { id: 'r23-le' })];
  game.player.hand = [c('tao', { id: 'r23-rep', suit: 'heart', color: 'red', rank: '5' })];
  for (let i = 0; i < 6; i += 1) game.deck.push(c('sha', { id: 'r23-pad-' + i }));
  game.deck.push(c('sha', { id: 'r23-orig', suit: 'spade', color: 'black', rank: '7' }));
  Engine.startTurn(game, 'player');
  assert.equal(game.pendingChoice && game.pendingChoice.kind, 'guicai-replace');
  const r = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'r23-rep' }));
  assert.equal(r.ok, true, r.message);
  assert.ok(game.discard.some((x) => x.id === 'r23-orig'), '鬼才代替: 原判定牌进弃牌堆');
