...
test('M1: 1 血曹操受杀致濒死 → 先桃自救脱离濒死, 后发动奸雄获得杀', () => {
  ...
  const result = Engine.playCard(game, 'player', 'lethal-sha');
...
test('M1: 曹操无救援牌死亡 → 奸雄不触发, 来源杀仍进弃牌堆 (牌守恒)', () => {
  ...
  const result = Engine.playCard(game, 'player', 'kill-sha');
  assert.equal(result.ok, true);
