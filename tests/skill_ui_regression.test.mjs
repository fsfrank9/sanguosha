...
test('engine exposes explicit skill implementation status for every catalog skill', () => {
  ...
  for (const required of ['wusheng', 'longdan', 'qingguo', 'paoxiao', 'jianxiong', 'ganglie', 'fankui', 'mashu', 'qicai', 'qianxun', 'tiandu', 'yiji', 'tieqi', 'tuxi', 'yingzi', 'kongcheng']) {
...
test('unimplemented skills are visible but explicitly disabled as todo, not silently clickable', () => {
  ...
  assert.equal(Engine.HERO_CATALOG.xuchu.skills[0].status, 'todo');
