...
test('J0-2 (张角三修): 延时锦囊无使用者 — AI 放置者被纳入无懈窗口但按立场保留自己的乐', () => {
  const game = buildGame();
  game.turn = 'enemy';
  ...
  // 张角三修: 延时锦囊判定"无使用者" → 放置者 (enemy AI) 不再被队列硬跳过,
  // 而是纳入窗口由 aiWuxieStance 判断 — 取消打在玩家的己方乐不符立场 →
  // 自动保留 (不乱耗)。玩家席无无懈, 乐照常判定。
  Engine.startTurn(game, 'player');
  assert.ok(game.enemy.hand.some((x) => x.id === 'e-wx'), 'AI 放置者按立场保留无懈 (不抵消自己的乐)');
  assert.ok(game.log.some((l) => l.includes('【乐不思蜀】判定')), '乐照常判定');
});

test('J0-2b (张角三修): 玩家放置的延时锦囊判定前, 玩家有无懈 → 被询问 (无使用者不跳过)', () => {
  const game = buildGame();
  game.turn = 'enemy';
  game.player.skillPreferences.wuxieResponse = 'ask';
  game.enemy.judgeArea = [c('lebusishu', { id: 'p-lbss', delayedSource: 'player' })];
  game.player.hand = [c('wuxie', { id: 'p-wx', suit: 'heart', color: 'red' })];
  stockDeck(game, 6);
  game.deck.push(c('sha', { id: 'j-black', suit: 'spade', color: 'black' }));
  Engine.startTurn(game, 'enemy');
  assert.ok(game.pendingChoice && game.pendingChoice.kind === 'wuxie-response',
    '玩家放置者在延时锦囊判定前被询问是否打出无懈 (此前作为来源被跳过)');
});

test('J0-2c (张角三修): AI 放置者 wuxiePolicy=always — 仍不取消自己打给敌方的延时锦囊', () => {
  const game = buildGame();
  game.turn = 'enemy';
  // AI (enemy) 放置乐于玩家判定区, 且开启 always 策略 + 手握无懈。
  // 立场过滤先于 always 早退 → 消己方打给敌方的乐不符立场 → 恒保留 (不自耗)。
  game.enemy.skillPreferences.wuxiePolicy = 'always';
  game.enemy.hand = [c('lebusishu', { id: 'e-lbss' }), c('wuxie', { id: 'e-wx' }), c('tao', { id: 'filler' })];
  stockDeck(game, 6);
  Engine.playCard(game, 'enemy', 'e-lbss');
  assert.equal(game.player.judgeArea.length, 1, '乐置入玩家判定区');
  Engine.startTurn(game, 'player');
  assert.ok(game.enemy.hand.some((x) => x.id === 'e-wx'),
    'always 策略下 AI 放置者仍按立场保留无懈 (不取消自己打给敌方的乐)');
  assert.ok(game.log.some((l) => l.includes('【乐不思蜀】判定')), '乐照常判定');
