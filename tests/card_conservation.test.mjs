...
const EQUIP_SLOTS = ['weapon', 'armor', 'horseMinus', 'horsePlus'];
...
test('H1: 丈八蛇矛主动使用 → 全场牌数守恒, 虚拟杀不进弃牌堆', () => {
  ...
  const result = Engine.playZhangbaSha(game, 'player', ['zb-a', 'zb-b']);
...
test('H1: 丈八蛇矛 vs 奸雄 → 曹操获得两张组成实体牌而非虚拟牌, 牌数守恒', () => {
  ...
  const before = countAllCards(game);
  const result = Engine.playZhangbaSha(game, 'player', ['jx-a', 'jx-b']);
  assert.equal(result.ok, true);
...
test('M5: 朱雀转化的【杀】被奸雄获得 → 手牌中还原为普通【杀】, 无临时标记残留', () => {
  ...
  const before = countAllCards(game);
  const result = Engine.playCard(game, 'player', 'zq-sha');
  assert.equal(result.ok, true);
  assert.equal(countAllCards(game), before, '奸雄获得后全场牌数不变');
...
test('H1 回归: 丈八蛇矛响应路径 (决斗) 牌数守恒不被破坏', () => {
  ...
  const before = countAllCards(game);
  const result = Engine.playCard(game, 'player', 'duel-card', { target: 'enemy' });
  assert.equal(result.ok, true);
