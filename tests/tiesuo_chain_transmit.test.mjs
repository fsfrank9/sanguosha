...
test('H4: 双方横置 + 火杀 → 目标受伤解除, 传导等量火伤给另一横置角色并解除', () => {
  ...
  const result = Engine.playCard(game, 'player', 'fs-1');
...
test('H4: 普通杀对横置目标 → 不解除、不传导', () => {
  ...
  Engine.playCard(game, 'player', 'ns-1');
...
test('H4: 目标横置但对方未横置 → 解除自身, 无传导', () => {
  ...
  Engine.playCard(game, 'player', 'fs-2');
...
test('H4: 未横置目标受属性伤害 → 完全无连环结算', () => {
  ...
  Engine.playCard(game, 'player', 'fs-3');
...
test('H4: 闪电雷伤传导 — 判定阶段 3 点雷伤传导给另一横置角色', () => {
  ...
  Engine.endTurn(game); // enemy 回合判定阶段闪电命中
...
test('H4: 传导火伤遇藤甲 → 火焰伤害 +1 正常结算', () => {
  ...
  Engine.playCard(game, 'player', 'fs-4');
...
test('H4: 传导伤害致死 → 游戏正常结束', () => {
  ...
  Engine.playCard(game, 'player', 'fs-5');
