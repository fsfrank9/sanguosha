...
test('v7 PR-13: 玩家受致命伤但有【桃】+ auto pref → 自救存活', () => {
  ...
  game.phase = 'play';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'killing-sha'));
...
test('v7 PR-13: 玩家致命伤但没有【桃】/【酒】→ 死亡', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'lethal-sha'));
...
test('v7 PR-13: 玩家有【酒】+ auto pref → 用酒(使用方法Ⅱ) 自救', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'killing-sha-2'));
...
test('v7 PR-13: 玩家有【桃】+ player default ask → pendingChoice "dying-rescue"', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'cause-lethal'));
...
test('v7 PR-13: resolve {cardId:tao-id} → 救活', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'rs-lethal'));
  const result = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'resolve-tao' }));
...
test('v7 PR-13: resolve {decline:true} → player 跳过响应；enemy auto → 不救 → 死', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'decl-lethal'));
  const result = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { decline: true }));
  assert.equal(result.ok, true);
...
test('v7 PR-13: AI 对手不救玩家 (responder !== dyingActor + auto → skip)', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'foe-lethal'));
...
test('C1+C2: 【酒】+【杀】对 1 体力玩家造成 2 点伤害 → hp 降至 -1 (不再 clamp 到 0)', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c1-jiu')); // 本回合下一张【杀】+1 伤害
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c1-sha')); // 2 点伤害命中
...
test('C2: ask 路径 — 第一张【桃】只回到 0 → 再次询问同一响应者出第二张【桃】 → 存活', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c2-jiu'));
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c2-sha'));
  assert.equal(game.player.hp, -1);

  // 第一张【桃】: -1 → 0, 仍未回到 1 → 应重新询问同一响应者 (再次 pendingChoice)
  const r1 = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'c2-tao-a' }));
  ...
  // 第二张【桃】: 0 → 1 → 存活
  const r2 = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'c2-tao-b' }));
...
test('C2: auto 路径 — 深度致命伤自动连出 2 张【桃】救回', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c2a-jiu'));
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c2a-sha'));
...
test('C1: 桃数量不足以抵消深度致命伤 → 死亡 (旧 clamp-to-0 下会错误存活)', () => {
  ...
  game.turn = 'enemy';
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c1d-jiu'));
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'c1d-sha'));
  // -1 → 用唯一的桃 → 0 → 无更多救援牌 → 死亡
