// v16 Y-G1(b): 闪窗守护迁往帧步骤，requestPlayerResponse 契约不变。
...
test('v9 PR-E25: 引擎 continueShaAfterCixiong — 玩家是杀目标 + shanResponse=ask 时暂停 (v10 V3 后走 requestPlayerResponse)', () => {
  ...
  const fn = engine.match(/function advanceShaResponses\(game, saved\)\s*\{[\s\S]*?\n\s{6}\}/);
  assert.ok(fn);
...
test('v9 PR-E25/E26: resolveShanResponseChoice — cardId 指定 / use 自动 / 否则不出', () => {
  ...
  assert.match(fn[0], /consumeResponse\(game,\s*targetActor,\s*'shan'/);
  assert.match(fn[0], /flows\.run\(game, 'sha', saved\)/);
});
