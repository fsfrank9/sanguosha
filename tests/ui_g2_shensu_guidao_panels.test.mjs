...
test('鬼道独立面板: 张角自己判定乐不思蜀 → guidao 面板 (措辞替换) → 选中黑牌确认 → 判定牌被替换', () => {
  ...
  assert.ok(game.player.hand.some((card) => card.id === 'ui-gd-orig'), '原判定牌被张角获得 (进手牌)');
  assert.ok(!game.discard.some((card) => card.id === 'ui-gd-orig'), '原判定牌不进弃牌堆');
  assert.ok(
...
test('鬼道独立面板: 装备区黑牌可作候选 (带装备前缀) → 替换生效, 装备离场', () => {
  ...
  // 张角三修: 鬼道 "替换" → 张角获得原判定牌 (进手牌)。
  assert.ok(game.player.hand.some((card) => card.id === 'ui-gd-orig3'), '原判定牌被张角获得 (进手牌)');
});
