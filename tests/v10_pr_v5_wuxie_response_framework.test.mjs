// v16 Y: 原无懈步骤迁到注册的 advanceWuxieResponses，守护跟随实现位置。
...
test('v10 V5: checkWuxieAndContinue 入口 + advanceWuxieChain 推进 + settleWuxieChain 结算', () => {
  ...
  assert.match(tricksSrc, /function advanceWuxieResponses\(game\)/);
...
test('v10 V5: advanceWuxieChain — 玩家 ask 走 requestPlayerResponse(kind:wuxie-response)', () => {
  const fn = tricksSrc.match(/function advanceWuxieResponses\(game\)\s*\{[\s\S]*?\n {4}\}/);
  assert.ok(fn);
...
test('v10 V5: resolveWuxieResponseChoice — cardId / use / 默认 decline 三分支', () => {
  ...
  assert.match(fn[0], /consumeWuxie\(game,\s*pending\.actor,\s*chain\.reason,\s*decision\.cardId/);
