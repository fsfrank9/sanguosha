...
test('D1: 鬼道 — AI 张角(enemy) 持黑牌自动替换玩家的判定, 获得原判定牌', () => {
  ...
  assert.ok(game.enemy.hand.some((x) => x.id === 'orig-judge'), '原判定牌被 AI 张角获得 (进手牌)');
  assert.ok(!game.discard.some((x) => x.id === 'orig-judge'), '原判定牌不进弃牌堆');
...
test('D2a: 鬼道 — 玩家张角(ask) 用黑牌替换自己的判定 → 替换生效', () => {
  ...
  // 张角三修: 玩家张角获得原判定牌 (进手牌)。
  assert.ok(game.player.hand.some((x) => x.id === 'orig-judge'), '原判定牌被玩家张角获得 (进手牌)');
  assert.ok(!game.discard.some((x) => x.id === 'orig-judge'), '原判定牌不进弃牌堆');
