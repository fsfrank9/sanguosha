...
test('缺陷3: 三席全横置, 火杀致环传导, 玩家仅一桃救一人 → 无 0血活人, 主公倒下终局', () => {
  ...
  assert.equal(res.ok, true, res.message);
  // ally 先被传导致濒死 → 玩家 ask 暂停; 用唯一的桃救 ally
  assert.ok(game.pendingChoice && game.pendingChoice.kind === 'dying-rescue', 'ally 濒死暂停等待救援');
  assert.ok(game.pauseState.chainTransmit, '传导队列保留待续跑 (player 尚未受传导)');
  Engine.resolvePendingChoice(game, { use: true, cardId: 'p-tao' });
  // 续跑: player 受传导致濒死, 无救 → 主公死 → rebelSide 胜, 终局
  const strandedAlive = game.seats.some((s) => game[s].hp <= 0 && game.phase !== 'gameover');
  assert.equal(strandedAlive, false, '无 0血却存活且游戏继续的搁浅角色 (修复前 player 搁浅)');
  assert.equal(game.phase, 'gameover', '主公倒下 → 终局');
  assert.equal(game.winner, 'rebelSide', '主公死 → 反贼方胜');
