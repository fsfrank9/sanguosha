...
test('L1: 玩家=反贼开局 — 轮转正确 + AI 主公先手 + 定时器链驱动到玩家回合', () => {
  ...
  // enemyStep 定时器链自动跑完 AI 座席回合, 轮到玩家。
  const flushes = driveTimers(300);
  assert.ok(flushes > 0, '定时器链确实被驱动');
  const after = UI.getGame();
  assert.ok(after.phase === 'gameover' || after.turn === 'player' || Engine.getPendingChoice(after),
    'AI 回合链推进至玩家回合/合法挂起/终局, 未卡死 (turn=' + after.turn + ')');
});
...
test('L2: 玩家自己回合内阵亡 — 渲染层自动接管驱动, 回合交给存活座席 (不卡死)', () => {
  const game = startIdentityWithRole('modeIdentity4Btn', null); // 缺省主公, 玩家先手
  driveTimers(300);
  game.turn = 'player';
  game.phase = 'play';
  game.pendingChoice = null;
...
test('L2: 亡席输入守卫 + 旁观文案', () => {
  const game = startIdentityWithRole('modeIdentity4Btn', null);
  driveTimers(300);
  game.turn = 'player';
  game.phase = 'play';
  game.pendingChoice = null;
