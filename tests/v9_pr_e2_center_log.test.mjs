...
// PR-E16: .pause-banner CSS 已删除. 此守护改测 .phase-prompt__brush
// (相同笔触风格, PR-E13 引入 + 仍在用).
test('v9 PR-E2: layout.css 含 brush 风格 (PR-E16 后 .pause-banner 删, .phase-prompt__brush 接管)', () => {
  const layout = fs.readFileSync(path.join(stylesDir, 'layout.css'), 'utf8');
  assert.match(layout, /\.phase-prompt__brush\s*\{/);
  const brushBlock = layout.match(/\.phase-prompt__brush\s*\{[\s\S]*?\}/);
  assert.ok(brushBlock);
  assert.match(brushBlock[0], /box-shadow:[\s\S]*?,[\s\S]*?,/);  // 至少 3 段笔触 box-shadow
});
