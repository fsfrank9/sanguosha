...
test('H1b 桃园: 双方受伤 + 无人无懈 → 双方各回 1 (回归: 行为不变)', () => {
  ...
  const r = assertCardConservation(game, () => Engine.playCard(game, 'player', 'ty'));
...
test('H1b 桃园: 双方受伤 + 对方持 1 张无懈(auto) → 抵消发动者回复, 对手仍自回', () => {
  ...
  const p0 = game.player.hp;
  const e0 = game.enemy.hp;
  const r = assertCardConservation(game, () => Engine.playCard(game, 'player', 'ty2'));
  assert.equal(r.ok, true);
...
test('H1b 桃园: 仅发动者受伤 + 对方无懈 → 发动者不回 (单目标)', () => {
  ...
  const p0 = game.player.hp;
  const e0 = game.enemy.hp;
  const r = assertCardConservation(game, () => Engine.playCard(game, 'player', 'ty3'));
  assert.equal(r.ok, true);
...
test('H1b 桃园 玩家 ask 路径: enemy 出桃园, player 用无懈抵消 enemy 的回复', () => {
  ...
  const p0 = game.player.hp;
  const e0 = game.enemy.hp;
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'ai-ty'));
  assert.ok(game.pendingChoice, '应暂停等玩家无懈 (针对 enemy 的回复)');
  assert.equal(game.pendingChoice.kind, 'wuxie-response');
  // 玩家打出无懈 → 抵消 enemy 的回复
  const r = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: 'p-wx' }));
  assert.equal(r.ok, true);
...
test('H1b 桃园: 双方满血 → 无目标, 直接结算 (集智仍可触发, 仅状态不变)', () => {
  ...
  const p0 = game.player.hp;
  const e0 = game.enemy.hp;
  const r = assertCardConservation(game, () => Engine.playCard(game, 'player', 'ty-full'));
  assert.equal(r.ok, true);
