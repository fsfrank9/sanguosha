...
test('红颜×鬼道: 接受改判 → 原黑桃判定牌花色还原后入弃牌堆 (无永久损坏)', () => {
  ...
  const orig = game.discard.find((card) => card.id === 'orig-spade');
  assert.ok(orig, '原判定牌进入弃牌堆');
  assert.equal(orig.suit, 'spade', '弃牌堆中的原判定牌花色已还原为黑桃');
