...
test('M2: 装新防具替换白银狮子 → 回复 1 点体力', () => {
  ...
  const result = assertCardConservation(game, () => Engine.playCard(game, 'player', 'new-armor'));
...
test('M2: 过河拆桥弃置对方白银狮子 → 对方回复 1 点体力', () => {
  ...
  assertCardConservation(game, () => Engine.playCard(game, 'player', 'gh-by'));
  const resolved = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { zone: 'equipment', cardId: 'by-guohe' }));
...
test('M2: 制衡弃置装备区白银狮子 → 回复 1 点体力', () => {
  ...
  const result = assertCardConservation(game, () => Engine.useSkill(game, 'player', 'zhiheng', ['by-zhiheng']));
...
test('M2: 反馈拿走对方装备区白银狮子 → 失去方回复 1 点体力', () => {
  ...
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'dmg-sha')); // 司马懿受伤 → 反馈 ask
  assert.equal(game.pendingChoice.kind, 'fankui-pick');
  const resolved = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { zone: 'equipment', cardId: 'by-fankui' }));
  assert.equal(resolved.ok, true);
...
test('M2: 满血时失去白银狮子 → 不回血 (回复体力受上限限制)', () => {
  ...
  assertCardConservation(game, () => Engine.playCard(game, 'player', 'rw-armor'));
