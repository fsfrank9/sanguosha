...
test('v8 PR-C1: playCardAs(lebusishu) → 方片进对手判定区, 类型为乐, 源牌从手牌移除', () => {
  ...
  game.player.hand.push(card);
  const result = assertCardConservation(game, () => Engine.playCardAs(game, 'player', 'p-diamond', 'lebusishu'));
  assert.equal(result.ok, true);
...
test('v8 PR-C1: 大乔 仍可走 wuzhong 直接使用方片无中 (国色不影响原牌使用)', () => {
  ...
  const result = assertCardConservation(game, () => Engine.playCard(game, 'player', 'p-wz'));
...
test('v8 PR-C1: 国色 触发后, lebusishu 判定流程仍正常 (非红桃 → 跳过 enemy 出牌阶段)', () => {
  ...
  game.player.hand.push(card);
  assertCardConservation(game, () => Engine.playCardAs(game, 'player', 'le-functional', 'lebusishu'));
