...
test('H1b-2 五谷: 双方无懈皆无 → 双方各获得 1 (回归: 行为不变)', () => {
  ...
  const r = assertCardConservation(game, () => Engine.playCard(game, 'player', 'wg0'));
...
test('H1b-2 五谷 玩家 ask: 玩家用无懈抵消对方(发动者)的获得', () => {
  ...
  assertCardConservation(game, () => Engine.playCard(game, 'enemy', 'ai-wg'));
...
test('H1b-2 五谷: 选牌暂停仍正常 (无懈窗口先结算, 对方无无懈)', () => {
  ...
  assertCardConservation(game, () => Engine.playCard(game, 'player', 'wg2'));
  ...
  const r = assertCardConservation(game, () => Engine.resolvePendingChoice(game, { cardId: pickId }));
  assert.equal(r.ok, true);
