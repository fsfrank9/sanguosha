...
| PR-E26 | 闪响应支持选用哪张牌当闪 (真闪 + 龙胆/倾国 转化候选) | 🟢 PR #94 已合并 |
| PR-E27 | 修 modal backdrop 拦截 hand-actions 确认/取消 点击 bug | 🟡 PR 待合并 |

总预估: **~2700-4200 LOC 变更**, 跨 27 个 review cycle。
...
- E27: 修 modal backdrop 拦截 hand-actions 确认/取消 点击 bug ✅
- E28 (计划): 万箭齐发 / 银月枪 的玩家闪响应 (PR-E25/26 只覆盖了【杀】路径)
- E29 (计划): 玩家手动【无懈可击】响应
- E30 (计划): guanxing / zhiheng / huogong / tiesuo modal 视觉升级卷轴风
...
### PR-E27 落地 — 修 modal backdrop 拦截确认/取消点击 bug ✅

**起因**: 用户反馈 PR-E26 后「有响应, 但是我用不了, 我只能选择不出闪」.

**根因**: `.pending-prompt-panel::before` 是全屏暗背景 (modal backdrop), 之前
`pointer-events: auto`. 该 `::before` 在 panel 的 stacking context (整体 z-40) 内,
盖在 `.hand-dock` (z-5) 之上 — `#handConfirmBtn` / `#handCancelBtn` 在 hand-dock
内, 被 backdrop 拦截了点击. 「不出【闪】」按钮在 panel **内部** (backdrop 之上)
所以能点 → 用户只能"不出闪". **影响所有两步化 pending modal** (E23/E24/E26 的
target/huogong/guicai/.../shan-response 的 #handConfirmBtn 都点不到).

**改动**:
- `modals.css`: `.pending-prompt-panel::before` `pointer-events: auto → none` —
  backdrop 改为纯视觉 (dim + blur 保留), 不拦截点击. hand-dock 内的确认/取消
  按钮恢复可点. 手牌误点已由 `_anyModalVisible()` 守护, 无需 backdrop 拦截.
- shan-response 面板 hint 文案补充引导 ("点选一张牌当【闪】后按下方【确认】").

**新增测试** `tests/v9_pr_e27_modal_backdrop.test.mjs` (3 条守护).

**Test status**: 849 → 852 ✓; `build:check` 通过.

**Process 遵守**:
- PR-E26 (#94) merge 状态已通过 `mcp__github__pull_request_read` 确认
  (`merged_at: 2026-05-16T11:48:57Z`) 后才从最新 main 拉新 branch `claude/v9-pr-e27-modal-backdrop-fix`.
