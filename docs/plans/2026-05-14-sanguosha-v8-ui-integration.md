...
| PR-C1 | 国色（大乔：方片 → 乐不思蜀 card-as） | 🟢 PR #55 已合并 |
| PR-C2 | 流离（大乔：杀转移；spec 忠实，1v1 候选恒空） | 🟢 PR #56 已合并 |
| PR-C3 | 急救（华佗：回合外红色当桃，接濒死流程） | 🟢 PR #57 已合并 |
| PR-C4 | 青囊（华佗：出牌阶段限一次，弃 1 手牌令受伤角色回 1 hp） | 🟢 PR #58 已合并 |
| PR-C5 | 洛神（甄姬：准备阶段连续黑色判定获得，方向 3 收尾） | 🟢 PR #59 已合并 |
...
| HOTFIX-2 | v8 PR-C1..C5 5 个新技能注册到 `IMPLEMENTED_SKILL_IDS`（之前 UI 一直显示"未实现"）+ qingnang `cardSkillConfig` + 洛神 prompt panel UI + 测试守护 | 🟢 PR #61 已合并 |
...
v8 方向 4 第四个 PR：把 lookahead 的 `aiEvaluateState` 升级为 `aiEvaluateStateWithThreat`，在 baseline 上扣减"对方下回合预期入帐伤害 × 25"。AI 因此会主动消除对手的攻击潜力（拆武器 / 顺手对方 sha 等）。

**痛点 (修复前)**：
- PR-D3 的 `aiEvaluateState` 只看 **当前** 局面 hp / 手牌 / 装备差异
- 不考虑对手手里那些 sha **下回合会攻击自己**
- 结果：AI 不会主动 disrupt 对手的进攻资源（明明顺手可以拿走对方 sha, 但 heuristic 评分认为不如出杀打掉 1 hp）

**改动 (`src/engine/game-engine.js`)**：

新增：
- **`aiEvaluateStateWithThreat(g, actor)`**:
  - baseline = `aiEvaluateState(g, actor)`
  - `gameover` 时跳过 threat (无意义), 直接返回 baseline
  - `oppSha = aiEstimateShaCount(opp)` (含武圣 / 龙胆 / 丈八等转化)
  - `selfShan = aiEstimateShanCount(self)` (含倾国 / 龙胆等转化)
  - `incoming = max(0, oppSha - selfShan)` (净入帐 dmg 估算)
  - 返回 `baseline - incoming * 25`
  - 25 ≈ 比 hp 差权重 30 略低, 不至于完全主导, 但能改变近平局的决策

集成：
- `aiScoreCardWithLookahead` 改用 `aiEvaluateStateWithThreat` 算 pre/post eval
- D3 的 `aiEvaluateState` 保留 (作为 baseline + 测试 + 可能未来扩展)

Engine 暴露: `aiEvaluateStateWithThreat`

**新增** `tests/ai_threat_aware.test.mjs`（8 条）：
- `aiEvaluateStateWithThreat` (6):
  - 对方手牌空 → 无威胁, 等同 baseline
  - 对方 2 sha + 我 0 闪 → penalty 50
  - 我 1 闪抵消 → penalty 25
  - 我闪数 ≥ 对方杀数 → 0 penalty
  - 对方装武圣红牌 也计入威胁
  - gameover 跳过 threat
- `aiScoreCardWithLookahead` 集成 (2):
  - 过河拆武器 → 提高分
  - shunshou 拆对方 sha 分数 > 南蛮 (高威胁场景偏好防御)

### PR-D3 落地 — 1-ply lookahead 框架

v8 方向 4 第三个 PR：引入"模拟 → 评估"框架。AI 决策时先 clone game state，模拟各候选牌出手后的局面，用 `aiEvaluateState` 给评分，与启发式 score 相加得到综合分。

**痛点 (修复前)**：
- AI 完全靠静态 `scoreCardForAI` heuristic — 不能识别"这张杀打出去对方就死了"这种致命牌
- 决斗 / 杀 / 锦囊 score 是基于 estimateShaCount/Shan 估算, 但 estimate 是粗略的 "数量" 不是 "结局"
- 多张高分牌候选时, AI 没法分辨"哪张实际效果更好"

**改动 (`src/engine/game-engine.js`)**：

新增辅助：
- **`aiCloneGame(g)`**: 深克隆 game (用 JSON), log/turnHistory 重置为空 (不必要), random 替换为独立确定 seed (避免污染原 game.random closure 状态)。返回带 `aiSimulating: true` 标记的 clone
- **`aiEvaluateState(g, actor)`**: 状态评估
  - hp 差 × 30 (主导)
  - hp ≤ 0 → -1000; hp = 1 → -50; hp = 2 → -10
  - hand 差 × 5
  - 装备件数差 × 8
  - 自己 judgeArea × -5; 对方 judgeArea × +5
  - gameover: actor 胜 +100000, actor 败 -100000
- **`aiSimulateCardPlay(g, actor, card, mode, options)`**: clone → playCard/playCardAs → 返回 sim game 或 null。null 表示 sim 失败 (pendingChoice 触发 / 异常 / 不合法)
- **`aiScoreCardWithLookahead(g, actor, card, mode)`**: 综合分 = heuristic + (postEval - preEval). sim 失败 → 仅 heuristic

集成：
- `aiChooseCard` 把 `scoreCardForAI` 调用改为 `aiScoreCardWithLookahead`. normal + asSha 两路径都走 lookahead

Engine 暴露: `aiCloneGame` / `aiEvaluateState` / `aiSimulateCardPlay` / `aiScoreCardWithLookahead`

**新增** `tests/ai_lookahead.test.mjs`（12 条）：
- aiCloneGame (1): 深克隆 + 独立 random + 不污染原 game
- aiEvaluateState (4): hp 差为主权重 / gameover ±100000 / 装备 +8/件 / 判定区 ±5
- aiSimulateCardPlay (3): 杀打中 → enemy hp -1 / 桃自救 → +1 / 触发 pendingChoice → null
- aiScoreCardWithLookahead (2): heuristic + delta / sim 成功复合分
- aiChooseCard 整合 (2): 致命杀优先 / 致命决斗优先

### PR-D2 落地 — pendingChoice auto 决策优化

v8 方向 4 第二个 PR：把 pendingChoice 的 `auto` 路径从原来 deterministic 的 `hand[0]` / `pool[0]` / `equips[0]` 改用 `scoreCardForAI` 排序后挑最优 / 最差牌。

**痛点 (修复前)**：
- `cixiong-choose` target auto：始终 `target.hand[0]` 弃置 — 受害者乖乖丢首张, 哪怕首张是桃
- `wugu-pick` picker auto：`pool.shift()` 取首张 — AI 不会挑最值钱的
- `guohe-1v1-pick` source auto：`equips[0]` 然后 `hand[0]` — 装备顺序按定义排, 拆什么完全看 slot 字典顺序
- `guicai-replace` holder auto：`holderState.hand[0]` — 鬼才 把首张投进判定堆 (可能是 桃!)

**改动 (`src/engine/game-engine.js`)**：

1. `cixiong-choose` target auto (line ~1538)：
   - 改 sort hand by `scoreCardForAI` ascending → discard lowest-score card
   - 效果：受害者保留 桃 / 无中 / 杀 等高价值牌, 弃掉 满血时的 桃 / 杀已用过等低分

2. `wugu-pick` picker auto (line ~1699)：
   - 改 iterate pool, track highest `scoreCardForAI`, 取最高分
   - 效果：受伤 picker 优先取 桃; 满血 picker 取 杀; AI 不再 lucky-dip

3. `guohe-1v1-pick` source auto (line ~1639)：
   - 改 装备 slot 优先级 `{ weapon: 1, armor: 2, horsePlus: 3, horseMinus: 4 }`
   - sort equips ascending, pick first
   - 效果：拆 武器 > 防具 > 马, 而非凭 `equipmentList` 顺序

4. `guicai-replace` holder auto (line ~929)：
   - 改 sort hand ascending → use lowest-score card as 判定 replacement
   - 效果：减少 鬼才 替换损失 (满血时的 桃 优先牺牲)

**新增** `tests/ai_pending_auto.test.mjs`（7 条）：
- 雌雄 target auto 弃 桃 (满血最低分)
- 雌雄 target auto critical hp 保留 桃, 弃次低
- 五谷 auto 受伤 player 取 桃
- 五谷 auto 满血 player 取 sha (不要 桃)
- 过河 auto 装多件时拆武器
- 过河 auto 无武器拆防具
- 鬼才 auto 用最低分手牌替换

更新 `tests/wugu_reveal_pick.test.mjs`: 原 "auto path: source 取顶张" 改为 "按 scoreCard 挑最高分", 因为新 AI 不再 deterministic 取首张。

### PR-D1 落地 — AI scoreCard 精细化 + 资源估算辅助

v8 方向 4 (AI 进阶) 开局 PR：把 `scoreCardForAI` 从 v6 binary heuristic 改成基于双方资源 + 自身受伤情况的梯度评分。

**痛点**：
- 桃 score 是 100 / -100 二元 — AI 在 hp=3 (轻伤) 和 hp=1 (危急) 时同分, 不优先自救
- 决斗 score 只数手牌里 sha 字面数量, 忽略 关羽 红牌当杀 / 赵云 闪当杀 / 张飞 paoxiao
- 杀 score 只看对方 hand 里 `c.type === 'shan'`, 不算 倾国 黑色当闪 / 龙胆 杀当闪
- 南蛮 / 万箭 / 过河 同样 binary

**改动**：

新增辅助 (engine 内, 非 destructive)：
- `aiEstimateShaCount(state)`: 计算 state 可作为杀使用/响应的牌数, 含路径：
  - 手牌 sha 字面
  - 武圣 (关羽): 红色非杀手牌 + 红色装备 (spec 允许装备区)
  - 龙胆 (赵云): 闪 → 杀
  - 丈八 (装备 zhangba): 剩余手牌 `floor(.../2)` 当杀 (排除已计入的)
- `aiEstimateShanCount(state)`: 同上但闪向, 含路径：
  - 手牌 shan 字面
  - 龙胆: 杀 → 闪
  - 倾国 (甄姬): 黑色非闪手牌 → 闪

`scoreCardForAI` 重写：
- 桃: 满血 -100; hp=1 critical 200; 多伤 120; 轻伤 80
- 杀: usedSha 锁定 -100 (除非 paoxiao); 0 闪 85; 1 闪 60; 2+ 闪 35
- 决斗: 用 `aiEstimateShaCount` 比较双方; 我多 75 / 持平 40 / 少 10
- 南蛮: 对方 `aiEstimateShaCount === 0` 80, 否则 30
- 万箭: 对方 `aiEstimateShanCount === 0` 80, 否则 30
- 过河: 对方 hand+equipment 总数 0 → -100, 3+ → 70, 中间 50
- 顺手 / 装备 / 延时 保持原值
- 无中 / 酒 保持原值 (前者总值钱, 后者依赖 sha)

`SanguoshaEngine` 暴露：`aiScoreCard` / `aiEstimateShaCount` / `aiEstimateShanCount` (供测试 + 后续 PR 复用)

**新增** `tests/ai_score_refinement.test.mjs`（21 条）：
- aiEstimateShaCount (4): 纯 sha / 武圣 / 龙胆 / 丈八
- aiEstimateShanCount (3): 纯闪 / 倾国 / 龙胆
- 桃 score 梯度 (3): 满血 / critical / 多伤 vs 轻伤
- 杀 score (4): 闪数量梯度 / usedSha / paoxiao / 对方龙胆转化计入
- 决斗 score (2): 双方杀数差 / 对手武圣红手降分
- 锦囊 (3): 南蛮 / 万箭 / 过河
- aiChooseCard 整合 (2): hp=1 桃优先, 对方多闪改出锦囊

### PR-C5 落地 — 洛神（甄姬：准备阶段连续黑色判定获得）

v8 方向 3 收尾技能：甄姬【洛神】。强力先手节奏 — 准备阶段可"白嫖"判定黑色牌入手。

**spec**（gltjk `card__hero__wei.md`）：
> 准备阶段开始时，你可以判定，若结果为黑色，你可以重复此流程。最后你获得所有的黑色判定牌。

**改动**：

数据层：
- `heroes.js` SKILL_METADATA：新增 `luoshen: { trigger:'preparePhase', frequency:'unlimited', cost:{ type:'judgement' }, hooks:['onPreparePhase'] }`

引擎层（接入 `processPreparePhase` 在 观星 之后）：
- `triggerLuoshenPrepare(game, actor)`：校验 hasSkill + pref（默认 `actor==='player' ? 'ask' : 'auto'`）；decline 则记日志返回；否则进 `startLuoshenStep`
- `startLuoshenStep(game, actor, pref)`：
  - ask + player → 设 `game.pauseState.luoshen = { actor }` + `pendingChoice.kind = 'luoshen-continue'` 暂停
  - auto/enemy → 调 `runLuoshenJudge`
- `runLuoshenJudge(game, actor, pref)`：
  - `judge(game, actor, '【洛神】')` 拉 deck top（含 onJudgementBeforeResolve）
  - 黑色 → push 到 actor.hand + 日志 → 递归 `startLuoshenStep`
  - 红色 → `discardCard` + 日志 + 清 pauseState.luoshen
- `resolveLuoshenContinueChoice(game, pending, decision)`：
  - `decision.decline || decision.stop` → 日志 + 直接 `continueTurnAfterPreparePhase`
  - 否则 → `runLuoshenJudge`；若再次 suspended，留 pendingChoice；否则 `continueTurnAfterPreparePhase`
- `resolvePendingChoice` 加 `'luoshen-continue'` 分支

UI 层：本 PR 不接入 UI panel（属于方向 1 范畴），可后续 PR-A6 补一个 `.pending-prompt-panel` 渲染（两按钮：继续 / 停止）。引擎层完备，可通过 `resolvePendingChoice({})` / `{decline:true}` 触发。

**新增** `tests/luoshen_continuous_judge.test.mjs`（10 条断言）：
- SKILL_METADATA 注册（preparePhase / judgement cost / onPreparePhase hook）
- 甄姬 auto + deck=[黑黑红] → 2 张黑色入手 + 红色入弃
- 首张就是红色 → 0 黑色入手 + 红色入弃
- pref=decline → 不触发 (top 牌作为摸牌阶段 draw)
- player ask → 首次设 pendingChoice (kind=luoshen-continue, actor=player)
- resolve({decline:true}) → 不判定 + 进入下一阶段 + 日志"停止洛神"
- resolve({}) → 黑色入手 + 再次 ask
- 连续 continue → 红色 → 自动结束 + 进入 judge
- 非甄姬（无技能）→ 准备阶段不触发
- AI enemy + pref undefined → 默认 auto

**已知限制（非本 PR 修复）**：
- 1v1 中 鬼才（司马懿）干预洛神判定走 `judge()` 的 `{pausable:false}` 缺省路径 → guicai 自动 fire（与 八卦阵 / 刚烈 判定一致），不会暂停。完整 pause 路径属于多人扩展。
- UI panel 未接入（仅引擎层）；待方向 1 PR-A6 补 luoshen-continue 渲染。

### PR-C4 落地 — 青囊（华佗：出牌阶段弃 1 手牌令受伤角色回 1 hp）

v8 方向 3 第四个技能：华佗【青囊】。出牌阶段主动技，配合急救双面板"医师"形象闭环。

**spec**（gltjk `card__hero__neutral.md`）：
> 出牌阶段限一次，你可以弃置一张手牌并选择一名已受伤的角色，令其回复 1 点体力。

**改动**：

数据层：
- `heroes.js` SKILL_METADATA：新增 `qingnang: { trigger:'playPhase', frequency:'oncePerTurn', cost:{ type:'discardOwn', count:1 }, hooks:['onActiveSkill'] }`
- `phases.js` `resetActorTurnState` / `resetEndOfTurnState` 添加 `flags.qingnangUsed = false`

引擎层：
- `triggerQingnangActiveSkill(context)`：
  - 校验 hasSkill + flags.qingnangUsed
  - cardIds[0] 必填（cost：弃 1 手牌）
  - target 从 `options.target` 解析（'player' / 'enemy' / 'self'）；未指定时：对方受伤则默认对方，否则自己；都满血 → fail
  - 目标必须 hp < maxHp（未受伤 fail）
  - 流程：removeCardFromHand → discardCard → target.hp = min(maxHp, hp+1) → flags.qingnangUsed=true
- SkillRuntime.registerSkill('qingnang', { onActiveSkill }) + 加入 `PLAY_PHASE_ACTIVE_SKILLS`
- AI（`aiChooseSkillAction`）：仅当自身受伤 + 有手牌时主动发动救自己（不给敌人回血），选最低分手牌做 cost

**新增** `tests/qingnang_heal_target.test.mjs`（10 条断言）：
- SKILL_METADATA 注册（playPhase / oncePerTurn / discardOwn 1）
- 自救：弃 1 手牌 → 自身 hp+1 + 标记
- 每回合限一次 → 第二次失败
- 目标未受伤 → 失败，手牌不弃
- 救对方：target='enemy' → 对方 hp+1
- 非华佗 → 失败
- 错误阶段（非 play） → 失败
- 错误回合（game.turn !== actor） → 失败
- 未指定 target 默认逻辑（对方受伤 → enemy；仅自身 → self）
- 回合切换 → qingnangUsed 复位，下回合可再用

更新 `tests/phase_runtime.test.mjs`：reset flags 期望集加入 `qingnangUsed: false`。

### PR-C3 落地 — 急救（华佗：回合外红色当桃）

v8 方向 3 第三个技能：华佗【急救】。1v1 真正实用 — 玩华佗的玩家被致命杀打到 hp=0 时，可用任何红色非桃非酒手牌救自己。
...
数据层：
- `heroes.js` SKILL_METADATA：新增 `jijiu: { trigger:'cardConvert', frequency:'unlimited', cost:{ type:'playHand', count:1 }, hooks:['attemptDyingRescue'] }`

引擎层（接入 PR-13 濒死流程）：
- `attemptDyingRescue` 增 jijiuCards 收集：`hasSkill(responder, 'jijiu') && game.turn !== responder` 时把手中红色非桃非酒的牌加入候选
- `pendingChoice.jijiuIds` 新字段（与 taoIds / jiuIds 并列）
- auto 路径救自己优先级：**桃 → 酒 → 急救红色**（自救才会到第三层；救他人 1v1 AI 不开）
- `executeDyingRescue` 增 `'jijiu'` kind：再次校验 hasSkill + 回合外 + 颜色，弃源牌 + dyingState.hp +1
- `resolveDyingRescueChoice` 把 jijiuIds 加入 `allowed` 列表；按 cardId 在哪个数组里决定 kind

UI 层：
- `dying-rescue` 面板（PR-A2 引入）render 时把 jijiuIds 加进 allIds，suffix 标 `· 急救`，title 写 "发动急救：将此红牌当桃救援"

**新增** `tests/jijiu_red_to_tao.test.mjs`（9 条断言）：
- SKILL_METADATA 注册
- 华佗 dying + 红色非桃手牌 + auto → 急救自救成功
- 桃 / 酒 / 急救 三层优先级（桃优先）
- 仅黑色手牌 → 不能急救 → 死
- 自己回合 dying（闪电）→ 急救限制回合外不触发 → 死
- player(华佗) + ask → pendingChoice.jijiuIds 含红色非桃非酒
- resolve {cardId: jijiuId} → 救活
- UI dom-adapter 读 jijiuIds + 渲染 `· 急救` suffix
- 非华佗 + 红色非桃手牌 → 急救不触发 → 死

### PR-C2 落地 — 流离（spec 忠实，1v1 候选恒空）

v8 方向 3 第二个技能：大乔【流离】。

**spec**（gltjk `card__hero__wu.md`）：
> 每当你成为【杀】的目标时，你可以弃置一张牌并选择你攻击范围内的一名角色，将此【杀】转移给该角色。
> ◆发动【流离】的目标角色须为此【杀】的使用者使用此【杀】的合法目标。

**1v1 限制（与用户确认 spec 忠实路径）**：
- 大乔的攻击范围内，除自己外只有源
- 源不能用杀指自己 → 不是合法目标
- 候选恒空 → 流离在 1v1 静默不触发

实现属于 *未来多人模式预留架构*：技能注册 + hook 入口完整，多人模式时把 actor 列表从 `['player','enemy']` 改成 `game.actors[]` 即可激活。

**改动**：

数据层：
- `heroes.js` SKILL_METADATA：新增 `liuli: { trigger:'shaTargetedAfter', frequency:'unlimited', cost:{ type:'discardOwn', count:1 }, hooks:['onShaTargeted'] }`

引擎层：
- 新增 `triggerLiuliOnShaTargeted(context)` helper:
  - 检测 hasSkill(target, 'liuli')
  - 必须有可弃牌（手牌 + 装备 + 判定区任一 ≥ 1）
  - 遍历 `['player','enemy']` 找候选：排除 target 自身 + 排除源（spec：源对自己不合法）+ 在 target 攻击范围内（`canReachWithSha`）+ 源此杀的合法目标（无 `cardTargetProtection`）
  - 1v1 候选恒空 → 返回 null
- 引擎注册：`SkillRuntime.registerSkill(skillRegistry, 'liuli', {onShaTargeted: ...})`
- 新增 `'onShaTargeted'` hook 类型 — 在 `playSha` 末尾、`continueShaAfterCixiong` 之前调用

**新增** `tests/liuli_sha_targeted.test.mjs`（8 条断言）：
- SKILL_METADATA 已注册 liuli（trigger + hooks 字段）
- 引擎注册 hook + helper 存在
- `playSha` 在 cixiong 后 runHook `'onShaTargeted'`
- 1v1 + 大乔成为目标 + 有手牌 → 流离不触发, 杀照常造伤
- 大乔无可弃 → helper 返回 null
- 非大乔 → 不触发
- 大乔闪挡杀 → 闪正常 (hook 不破坏响应)
- daqiao hero 仍带 liuli skill descriptor

**多人模式扩展点**：在 `triggerLiuliOnShaTargeted` 里需要的额外工作：
1. pendingChoice 'liuli-decision'（让大乔选择弃哪张牌 + 选哪个候选）
2. `resolveLiuliDecisionChoice` 处理弃牌 + transfer
3. transfer 后 sourceCard 的 target 改成 newTarget，重新进入 `continueShaAfterCixiong`
...
| `tests/liuli_sha_targeted.test.mjs` | 8 | PR-C2 |
| `tests/jijiu_red_to_tao.test.mjs` | 9 | PR-C3 |
| `tests/qingnang_heal_target.test.mjs` | 10 | PR-C4 |
| `tests/luoshen_continuous_judge.test.mjs` | 10 | PR-C5 |
| _方向 3 小计_ | **46** | C1..C5 |
| `tests/ai_score_refinement.test.mjs` | 21 | PR-D1 |
| `tests/ai_pending_auto.test.mjs` | 7 | PR-D2 |
| `tests/ai_lookahead.test.mjs` | 12 | PR-D3 |
| `tests/ai_threat_aware.test.mjs` | 8 | PR-D4 |
| _方向 4 小计 (进行中)_ | **48** | D1, D2, D3, D4 |
| _其余 PR 待开_ | — | — |
