...
| PR-15 | 【方天画戟】最后一张手牌时 +2 额外目标标记 | `card__equipment.md` "若你使用的【杀】是最后的手牌..." | 🟢 PR #41 已合并 |
| PR-16 | 【无中生有】target 可选（含对手） | `card__scroll.md` "包括你在内的一名角色" | 🟡 PR 待合并 — v7 series 收尾 |
...
**1v1 实际意义**：把无中生有 用在对手身上 = 反直觉操作（让对手多摸 2 张），AI 永不这样做；保留 API 仅为 spec 合规。

---

## v7 series — 完整收尾汇总

至此 v7 牌效与流程规则 spec-compliance series **16/16 完成** (PR #27..#41 + #42)。

### 落地 PR 时间线

| Tier | PR | 修复对象 | 合并号 |
|---|---|---|---|
| 1 | PR-1 | 【桃】target 选择 | #27 |
| 1 | PR-2 | 【桃园结义】对未受伤角色无效 | #28 |
| 1 | PR-3 | 【麒麟弓】弃 1 匹马 + pendingChoice | #29 |
| 1 | PR-4 | 【雌雄双股剑】完整 spec (性别 + 二选一 + 指定目标后) | #30 |
| 1 | PR-5 | 【借刀杀人】双合法性检测 | #31 |
| 1 | PR-6 | 延时锦囊同名禁叠 | #32 |
| 1 | PR-7 | 【五谷丰登】reveal-then-pick | #33 |
| 2 | PR-8 | 【酒】每回合限一次 + shaBonus 绑回合 | #34 |
| 2 | PR-9 | 【过河拆桥】1V1 二选项 | #35 |
| 2 | PR-10 | 【顺手牵羊】1V1 无距离 | #36 |
| 2 | PR-11 | 【兵粮寸断】1V1 无距离 | #37 |
| 2 | PR-12 | 【闪电】next-valid 链转 | #38 |
| 3 | PR-13 | 濒死流程 (Tier 3 基础) | #39 |
| 3 | PR-14 | 【丈八蛇矛】响应路径 | #40 |
| 3 | PR-15 | 【方天画戟】触发标记 | #41 |
| 3 | PR-16 | 【无中生有】target 可选 | #42 |
...
| 合计 | **116** 条 spec-compliance behavior 测试 | |

### 已知遗留 / 后续方向

- **UI 面板**: 7 个新 pendingChoice (qilin-pick / cixiong-* / jiedao-decision / wugu-pick / guohe-1v1-pick / dying-rescue) 的 dom-adapter 面板还没接；玩家通过 `resolvePendingChoice({...})` API 走代码路径可用，UI 未给入口。后续 UI PR 统一补 panel。
