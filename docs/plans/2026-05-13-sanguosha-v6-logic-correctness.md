...
> 实际后续路径：方向 (3) "牌规则对照官方" 走通了完整 series，演化成 **v7**（16 PR / 116 测试，详见独立 plan 文档）。

---

## 后续工作 → 见独立 plan 文档

本文档收录 **v6.0 → v6.1** 阶段。2026-05-14 起后续工作按主题拆分到独立 dated 文件，避免单一文档无限膨胀：

- **v7** — 牌效与流程规则对照 gltjk spec-compliance series（16 PR，#27..#42，全部已合并）
  → [`docs/plans/2026-05-14-sanguosha-v7-card-rule-compliance.md`](./2026-05-14-sanguosha-v7-card-rule-compliance.md)
- **v8** — UI 集成 + 装备扩展 + 技能扩充 + AI 进阶（进行中，已落 PR-0 = #44）
  → [`docs/plans/2026-05-14-sanguosha-v8-ui-integration.md`](./2026-05-14-sanguosha-v8-ui-integration.md)

未来 v9（多人模式 / 国战）启动时会再开新文档。
