# 版本演进史

> 本文件收纳从 README 移出的历史细节。各版本的完整计划与执行记录见
> `docs/plans/` 对应文档；本文只保留"发生了什么"的浓缩档案。

## v4 — 安全拆源（已冻结）

v4.0 不是重写，而是分批"安全拆源"：保留根目录 `index.html` 作为可直接打开的
稳定产物，把 CSS、数据模块、引擎、UI 适配层逐步抽到 `src/`，用
`tools/build.mjs` 生成产物，用 `tests/architecture_build.test.mjs` 等防止源码
与产物漂移。

Phase 4A–4T 把 26 个已实现技能逐个迁入 `SkillRuntime` hook seam（每个 Phase
一个技能/一组技能，保持行为零漂移）：

| Phase | 技能 | seam |
|-------|------|------|
| 4A | 闭月 | `onTurnEnd` |
| 4B | 克己 | `onBeforeDiscardPhase` |
| 4C | 集智 | `onCardUse` |
| 4D | 英姿 | `onDrawPhase` |
| 4E | 突袭 | `onDrawPhase` |
| 4F | 咆哮 / 马术 | `hasPassiveEffect` / `sumPassiveEffect` |
| 4G | 空城 | `onCardTarget` |
| 4H | 铁骑 | `onNeedResponse` |
| 4I | 奸雄 | `onDamageAfter` |
| 4J | 武圣 / 龙胆 | `onCardAs` |
| 4K | 制衡 / 苦肉 / 仁德 / 反间 / 观星 | `onActiveSkill`（观星预览走 `onSkillPreview`） |
| 4L | 倾国 | `onCardAs`（响应转化） |
| 4M | 奇才 | `hasPassiveEffect('ignoreTrickDistance')` |
| 4N | 谦逊 | `onCardTarget` |
| 4O | 天妒 | `onJudgementAfterResolve` |
| 4P | 刚烈 | `onDamageAfter` + judgement finalizer |
| 4Q | 反馈 | `onDamageAfter` |
| 4R | 遗计 | `onDamageAfter`（逐点摸牌） |
| 4S | 裸衣 | `onDrawPhase` + `onDamageModify` |
| 4T | 鬼才 | `onJudgementBeforeResolve` |

详见 `docs/plans/2026-04-29-sanguosha-v4-architecture.md`。

## v5 — 原生 ES 模块 + GitHub Pages（已完成）

Phase 5A–5E 完成「单文件 IIFE + window globals → 原生 ES 模块 + GitHub Pages
静态托管」全量迁移。`dist/` 与单 HTML 产物不再维护，`src/` 即浏览器加载的
资产本身。详见 `docs/plans/2026-05-13-sanguosha-v5-architecture.md`。

## v6.0 — 数据驱动 + 暂停/恢复机制

- **结构化技能元数据**（`src/data/heroes.js` 的 `SKILL_METADATA`）：每个已实现
  技能各有 `{ trigger, frequency, optional, mandatory, cost, hooks }` 六字段，
  跨武将共享的同名技能（mashu / wusheng / longdan / biyue / paoxiao）自动保持
  一致；UI tooltip 从结构化字段派生。
- **结构化牌规则**（`src/data/cards.js` 的 `CARD_RULES`）：每张牌各有
  `{ summary, timing, targets, effect, frequency, responseWindow, engineHooks }`，
  自动 merge 到 `CARD_CATALOG[id].rule`。
- **官方规格 cache + audit harness**：
  `official-skill-cache/sanguosha-standard/official_standard_skill_cache.json`
  保存已实现技能的官方规格副本（带 sourceTextSha256）；
  `tests/v6_skill_audit.test.mjs` + `tests/skill_schema.test.mjs` +
  `tests/card_rules.test.mjs` 持续校验「cache ↔ specs fixture ↔ heroes.js ↔
  cards.js」四方一致。
- **装备被动效果注册表**（`src/engine/state.js` 的 `EQUIPMENT_EFFECTS`）。
- **玩家选择暂停/恢复机制**：`game.pendingChoice` + `game.pauseState` + 公开
  API `setSkillPreference` / `getSkillPreference` / `getPendingChoice` /
  `resolvePendingChoice`。
- **AI 主动技能感知**：AI 识别全部 5 个主动技 + 2 个出牌期转化（武圣/龙胆），
  其余被动/触发/锁定技通过引擎 hooks 自动生效。

## v6.1 — per-skill spec audit 修复链（7 个 PR）

v6.0 收尾后"逐字对照"官方 spec，发现 12 处真实 mismatch，7 个 PR 修完：

- **观星 #17**：预览张数 = min(存活角色数, 5, deck)；触发时机搬到准备阶段；
  `{ topIds, bottomIds }` 支持任意排序 + 顶/底独立分配。
- **鬼才 #18**：跨 actor 触发（任何判定都能干预）；非 pausable 判定回退 auto-fire。
- **反间 #19**：目标方真·猜花色；player UI 弹 4 花色选择；AI 盲随机。
- **反馈 #20**：玩家挑区域 + 具体牌。
- **刚烈 #21**：可选触发 / 来源选弃 2 vs 受 1 / 选哪两张 / 含装备可弃。
- **武圣+制衡+苦肉 #22**：装备区可作牌源；制衡可弃装备；苦肉 hp=1 允许。
- **突袭+遗计 #23**：突袭可不发动；遗计按伤害点数逐点处理。

累计基础设施：7 个 pendingChoice kinds、6 个 skillPreferences toggles、
3 个 "自己的牌 = hand + equipment" helpers（`findOwnCardById` /
`removeOwnCardFromAnyZone` / `firstMatchingOwnCard`）、判定 `pausable` flag、
多点伤害逐点处理、60 条新 behavior 测试。
详见 `docs/plans/2026-05-13-sanguosha-v6-logic-correctness.md`。

## v7 — 牌规则合规（16 个 PR #27–#42）

杀/闪响应窗口、桃/酒限制、距离与范围、即时/延时锦囊交互、装备特效
（青釭/诸葛连弩/方天/麒麟/古锭/雌雄/寒冰/朱雀）、濒死流程。
详见 `docs/plans/2026-05-14-sanguosha-v7-card-rule-compliance.md`。

## v8 — UI 集成 + 标准包技能扩充

补齐大乔【国色】【流离】、华佗【急救】【青囊】、甄姬【洛神】
（已实现技能 26 → 31）；AI 评估 1-ply lookahead + 威胁感知。
详见 `docs/plans/2026-05-14-sanguosha-v8-ui-integration.md`。

## v9 — UI 全面改版

布局重构、cream 卷轴风、modal/响应两步式、CSS 拆分为 10 个文件。
详见 `docs/plans/2026-05-14-sanguosha-v9-ui-overhaul.md`。

## v10 — 稳定化与扩展

响应窗口框架（无懈/万箭/银月/决斗杀统一暂停/恢复）、dispatch 注册表、
card-as 一致性。详见 `docs/plans/2026-05-28-sanguosha-v10-stabilize-and-expand.md`。

## 一轮审计修复（post-v10，PR #105–#113）

逐条对照 `official-skill-cache/gltjk-sanguosha-rules` 的全方位审计：

- **濒死流程 (#105)**：体力值可降至负数；同一响应者可连续出多张【桃】/【酒】
  直到回复至 1 点。
- **无懈可击覆盖 (#106)**：无中生有 / 南蛮入侵 / 万箭齐发 / 延时锦囊放置补齐
  无懈窗口。
- **八卦阵对万箭 (#107)**：万箭的【闪】响应补八卦红判定兜底。
- **朱雀羽扇 (#108)**：转火杀改为「本次使用」的临时视为，弃置时还原物理牌身份。

审计纪要见 `docs/audit/2026-06-09-code-audit-and-remediation.md`。

## 二轮审计修复（PR #115–#123）

第二轮独立代码审计后的修复，按批次拆 PR：

- **丈八蛇矛牌守恒 (#115)**：虚拟【杀】不再凭空进弃牌堆；奸雄改为获得组成
  实体牌；朱雀临时火杀入手前还原身份。
- **暂停/恢复架构闭环 (#116)**：pendingChoice FIFO 队列化；判定阶段濒死冻结
  回合流程；挂起时冻结全部公开入口。
- **伤害后时机 (#117)**：奸雄/反馈/刚烈/遗计移到濒死结算之后；死亡后技能
  不再触发。
- **铁索连环传导 (#118)**：横置角色受属性伤害解除连环并传导等量同属性伤害。
- **白银狮子 + 判定区归属 (#119)**：任何失去白银狮子的路径都回血；寒冰剑/反馈
  不再把判定区牌当作角色的牌。
- **牌规则杂项 (#120)**：五谷逐张亮出按需洗牌；贯石斧可选发动且成本可含装备；
  火攻展示牌由目标选择；奸雄可获得决斗/南蛮/万箭/火攻实体牌；国色转化乐走
  无懈链；discardExcess 事务化。
- **文档与工程卫生 (#121)**：README 数字、版本标记、Node 下限、CI 双跑、
  Pages 产物瘦身。
- **UI 面板补全 (#122)**：贯石斧自选两张 + 火攻展示牌自选面板，UI 对局默认 ask。
- **UI 行为测试基建 (#123)**：零依赖 fake-DOM 垫片 + dom-adapter 首批 8 条
  全链路行为测试。

## 已实现技能引擎接入说明（31 个）

主动/交互技能（7 个）：孙权【制衡】、黄盖【苦肉】、刘备【仁德】、周瑜【反间】、
诸葛亮【观星】、华佗【青囊】、甄姬【洛神】。

转化/被动/自动技能：

- 【闭月】：貂蝉结束阶段摸 1 张牌；`endTurn` 与阶段推进路径都触发。
- 【克己】：本回合未使用/打出/响应过【杀】时跳过弃牌阶段。
- 【集智】：成功使用普通锦囊后摸 1 张；响应【无懈可击】成功抵消也触发；
  非法使用、非普通锦囊、铁索重铸不触发。
- 【英姿】：摸牌阶段额外摸 1 张（`onDrawPhase` seam）。
- 【突袭】：对手有手牌时获得 1 张并少摸 1 张。
- 【咆哮】/【马术】：无限【杀】与出距 -1，走被动效果 seam。
- 【空城】：无手牌时不能成为【杀】/【决斗】目标。
- 【铁骑】：杀指定目标后判定，红色令目标不能打出【闪】。
- 【奸雄】：获得造成伤害的实体牌（含决斗/南蛮/万箭/火攻；虚拟合成牌取组成
  实体牌；在濒死结算之后触发）。
- 【武圣】/【龙胆】：红牌当杀 / 杀闪互转，含装备区牌源与响应窗口。
- 【倾国】：无真实【闪】时黑色手牌当【闪】响应。
- 【奇才】：使用距离受限锦囊时忽略距离限制。
- 【谦逊】：不能成为【顺手牵羊】/【乐不思蜀】目标。
- 【天妒】：自己的判定牌结算后获得之。
- 【刚烈】：受伤后判定，非红桃时来源弃两张（可含装备）或受 1 伤；在濒死
  结算之后触发（死亡不触发）。
- 【反馈】：受伤后从来源手牌/装备区获得一张牌（判定区牌不可获得）。
- 【遗计】：每受 1 点伤害摸两张，按点逐点 prompt 分配。
- 【裸衣】：摸牌少 1，本回合【杀】/【决斗】伤害 +1。
- 【鬼才】：判定牌生效前可用手牌替换（跨 actor）。
- 【国色】：方片当【乐不思蜀】（走 delayed-place 无懈链）。
- 【流离】/【急救】：见 v8 计划文档。

> 注: 以上为 v8 时代的 31 技清单快照; v11 (31→40) 与 v12 H (→53) 新增技能的
> 接入说明见 `docs/plans/2026-06-09-sanguosha-v11-roadmap.md` 与
> `docs/plans/2026-07-05-sanguosha-v12-roadmap.md`。个别条目描述已被后续
> 审计修正 (如刚烈成本 v13 起限手牌、突袭 v13 起按官方"放弃摸牌"语义、
> 遗计 v13 起逐席分配), 以 v13 审计纪要为准。

## v11 — 守恒硬化 + 域拆分 + AI 期望值决策（#125–#150）

全场牌数守恒 census、引擎域模块拆分（damage-dying/response/tricks/
equipment/judge-area）、技能 31→40、AI 期望值决策与无懈 EV。详见
`docs/plans/2026-06-09-sanguosha-v11-roadmap.md`（含收尾盘点）。

## v12 — 双模式收官（F/G/H/I 四阶段）

路线图 `docs/plans/2026-07-05-sanguosha-v12-roadmap.md`。如实进度
（修复批核对后收官）：

1. **F 结构减重**：F1 技能域、F2 判定区、F3 PLAY_HANDLERS、F4 大厅面板、
   F5 牌结算链（杀链 sha-flow.js + 锦囊结算入 tricks.js）、F6 战场渲染域
   全部拆出；game-engine 1941 行（≤2200 ✓）/ dom-adapter 1135 行（≤1200 ✓）。
2. **G 扩展包技能**：风包 9/11 接入（据守/烈弓/狂骨/神速/红颜/天香/雷击/
   鬼道/不屈），fixture 扩至 8 将 11 技；黄天按计划随 H 激活跨"≥50"验收线，
   蛊惑留待多人评估。
3. **H 多人模式（3 人身份场，含 UI）**：座次环结算（无懈队列/AOE 逐席/
   决斗跨座/铁索环/闪电环/桃园五谷座次）、全牌类显式目标 + 座席级合法目标
   矩阵、identity3 距离规则、身份死亡结算与奖惩、主公技激活（激将/护驾/
   黄天）+ 离间（技能 49→53）、AI 阵营立场；三席 UI（模式选择/第三席渲染/
   座席点选/求助面板）。全程 1v1 行为零回归。
4. **I AI 进阶**：整回合深度模拟（胜率主引擎，独立贡献 ~+6-7pt）、可见
   信息诚实计数（去全知 + 响应空窗推断）、多人目标评估（敌意记账/胜负手/
   收割）、启发式状态机。验收：v12 对 v11 冻结基线固定种子自对弈，验收段
   61.0% / 四独立段 700 局合计 57.6%（≥55% ✓，tests/v12_i_benchmark
   常驻门禁）。

## v13-J — 清账与全局合规盘点（PR #167）

玩家实测 4 例定点缺陷修复（出牌暂存-确认 / 延时锦囊无懈时机迁至判定
生效前 + 判定阶段 LIFO / 藤甲仁王盾免疫短路与八卦先行 / 桃收口为仅自己
已受伤）；v12 记录在案三项清零（遗计逐席分牌 / 火攻成本挂起重选 / 天香
真 ask 伤害流暂停框架 + 伤害落点回调）；第三轮全量合规审计（8 并行维度 +
逐条对抗性复核）22 条确认 → 20 修复 + 2 如实降级；收官前 8 角度 code
review 再修 8 处。风包 gid 因环境网络策略如实降级。全程 1v1 与 3 人身份
场双红线保持。完整纪要：
`docs/audit/2026-07-16-third-round-compliance-audit.md`；执行记录：
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-K — 5 人身份场与内奸

4/5 人身份场全量落地：身份预设 4 人档（主/反/忠/内）与 5 人档（主/反/
忠/反/内）转正、内奸胜负条款（主公亡时仅剩内奸 → 内奸单独获胜；主公方
获胜收紧为反贼与内奸全灭）、死亡奖惩与回合环 4/5 席行为测试。结算加压
自审修出 3 处多席活跃缺陷（国色/奇袭转化目标硬编码 opponent() 错指玩家
席、铁索缺省目标二元残留、濒死队列亡席陪跑），16 例固定种子加压测试
入库。audit backlog 三项销账：advanceTargetQueue 逐目标无懈驱动泛化
（四份同型收敛、行为零变化）、酒使用方法Ⅰ他指（card__basic.md:58，
shaBonus 挂目标 + 回合结束全席过期）、天香/火攻面板 DOM 级测试（连带
发现并修复 huogongCostChoices id 重复 — 真实浏览器下 J2 重选候选渲染
进隐藏旧面板）。UI 座席环：预置槽位（ally2/ally3）+ setup 4/5 人模式 +
内奸紫徽章 + 按 game.seats 泛化渲染；修复身份场终局横幅恒显"败北"的
既有缺陷。K4 基准：4/5 席固定种子全 AI 自对弈入 verify 门禁（逐步守恒/
零挂起泄漏/终局可达，~2s）。内奸 AI 全敌对为已知简化（留 M 阶段身份
价值评估）。1v1 与 3p 双红线零回归。执行记录：
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-L — 可选身份与阵亡旁观

setup 身份选择五档 (主/忠/反/内/随机, 预设阵型内轮转构成不变); 引擎
firstActorFromRoles 全环扫描修复 (主公落任意座席先手正确, 旧实现只查
player/enemy 字面键); AI 主公先手经 enemyStep 定时器链自动接管; 黄天
给牌死钮回归; 激将/护驾双向化 4/5 席验证; 三席内奸徽章与文案中性化。
玩家阵亡旁观: 自己回合内阵亡不再卡死 (亡席回合按 AI 驱动 + render
幂等接管), 亡席输入守卫, 旁观横幅, 终局按真实身份结算。救援
play-phase 复议裁定维持 J0-4 桃收口。矩阵测试 8 格 + soak 可选身份
3 局 + 全仓库首个 flushTimers 时序 UI 测试。执行记录:
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-M — 暗身份与推断 AI

暗身份转正为可选模式 (官方规则: 除主公外身份牌死亡亮出前不可见, 既有
明置降级为记录在案的简化): hiddenRoles + roleRevealed 逐席可见性表,
死亡翻明先于胜负判定, 终局全翻明, UI 问号徽章与 setup 开关。去全知
第二轮 (I2 方法论): 规则层 (胜负/奖惩/求助资格/救援+1) 保持真实身份
直读, AI 知识层统一 perceived* 感知路由 — 明置恒等 (零回归), 暗置仅
自己/已翻明直读, 未知按敌对缺省; 零直读守护测试常驻 (逐文件计数锁定,
白名单 4 处皆主公公开或规则层) + 零全知行为断言 (未翻明座席身份互换
决策不变)。推断模型: aggressionLog 伤害方向 + stanceLog 立场遥测
(救援/无懈方向/求助响应, aid4/rescue3/wuxie2) → inferredLeaning 阈值
±2 判读; 击杀反贼奖励暗置折半 (信念非事实); 内奸骑墙初版 (感知阵营
聚合战力, +15 打压强势侧 — K4 简化销账)。M4 基准入 verify 门禁: 暗置
6 局 soak (守恒/终局可达/全翻明) + 推断准确率 5p 64.1% vs 随机基线
37.5%、4p 60.1% vs 33.3% (门禁 ≥50%/≥45%); 明置同种子对照 6 局 3 局
终局翻转 (胜率差可测)。执行记录:
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-N — 内容评估批

三子项裁决收官: 蛊惑 (于吉) 评估后本批不接入 — spec 文本完整 (gltjk
镜像含流程与判例) 但两个官方版本实质分歧无裁定基准, 且为横切全部
基本/锦囊结算路径的虚拟声明牌层 (成本≈独立阶段), 记录为立项候选;
军争盘点 — 官方缓存无任何牌数/花色/点数表 (该维度如实降级), 名录
对照军争零缺口 (8 张军争标签牌全在库; 缺席 9 名均非军争 — 1v1/
国战/界限突破专属), 销账一处真实可达性
缺口: 寒冰/古锭/朱雀/银月规则齐全却漏 buildDeck 配方 (实战不可摸),
各 ×1 入堆 (138→142) + 目录⇄配方守护测试常驻; 配方重排令 k4 soak
守恒红线抓获预存 bug — 奸雄从弃牌堆取回多目标 AOE 来源牌后, 后续
席位收尾误补弃致双区并存 (AOE 为唯一同来源牌多席结算面) → 已归属席位不补弃 + 回归
钉死; 基准重测 200 局胜率 62.5%、M4 推断 5p 54.9%/4p 47.9%, 门禁
全数保持。火/林/山无 spec 源, 零接入维持。执行记录:
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-UI修缮 — 用户实测反馈 6 项

出牌确认统一 (铁索面板/重铸/苦肉三处直出清零, 全 UI 无零确认结算
入口); 帮助文案随现状更新; 菜单未实现占位项移除; 一级界面分入口
(1v1/身份场, 二级选人数, 缺省 5 人档); 暗身份默认开启 (开关保留);
角色卡单卡化 — 装备定序四槽纵列 + 延时锦囊名首字着色圆标 (乐/兵/闪)
+ 手牌数角标, AI 牌背行撤销 (隐私更严), 多人场 AI 席同行多列解压。
执行记录: `docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-UI修缮二批 — 二轮实测 4 项 (真浏览器验证引入)

首次引入 Playwright 真浏览器验证 (Chromium 驱动真实页面, fake-dom 盲区
补位): 出牌确认硬保证 (真浏览器 19 牌型逐一审查后删除条件式直出
fallback, 直出仅存弃牌勾选/技能选牌两处受控模式, e16 守护锁定);
对手卡恒定尺寸 (236px 定宽 + 席位行 auto 高, 不随人数缩放); [hidden]
全局兜底 (真浏览器抓根因 — 显式 display 类覆盖 UA 默认, tokens.css
!important 兜底); 号位选将 (进选将前掷定逐席身份, 主公=1号位沿座次环
编号, 全席位格子选将, 开局消费同一份身份表 — 所见即所得)。执行记录:
`docs/plans/2026-07-16-sanguosha-v13-roadmap.md` 尾部。

## v13-UI修缮三批 — 三轮实测 6 项

下拉行全退役 (格子选将覆盖全席位); 随机键逐席可用 (randomizeHero 泛化
currentPickSide); 已选武将全席置灰锁定; 装备槽排版放松; 坐骑实名
(官方实名 赤兔/的卢/爪黄飞电 等经 nameVariants 逐份轮配, 引擎按 type
结算、名字纯展示); 无懈帮人明示 (目标为他人时面板明示"可打出无懈为其
解围", 补齐文案缺口 — 引擎两时机本就询问)。执行记录: 同上。

## v13-武将图鉴及续批

首页「武将」入口转正 (五个 disabled 导航项之一): 图鉴屏按 HERO_CATALOG
× 实现状态动态渲染 — 四势力分组, 每技能标注 已实现/未实现 (主动技加
标), 汇总计数与数据源恒同步。续批 2 项: 导航光标按 :disabled 区分
(已启用项不再顶禁止光标); 图鉴与选将界面加 全部/魏/蜀/吴/群 阵营筛选
chip (纯浏览辅助, 点选/随机/互斥逻辑不受影响)。执行记录: 同上。

## v13-张角修缮 (一修) — 实测 4 症状

前三症状同根: 雷击玩家侧无询问直发 + 内嵌判定不可挂起令玩家鬼道 ask
落 auto 兜底 (把自己雷击的黑桃判定换成最低分黑牌 → "黑桃不命中")。
修复: 雷击询问化 (deferred-ask 面板, 暂存-确认, FIFO 保触发相对序);
玩家改判禁 auto 兜底 (鬼道/鬼才在不可挂起判定时机明示跳过, 不替玩家
烧牌); 雷击判定可挂起 (pauseState.leiji 独立快照, 打开"判定非黑桃 →
鬼道补黑桃"的改判窗口); AI 鬼道护栏 (仅原判定非黑桃+手有黑桃+目标无
红颜时替换); 雷击/鬼道描述修正。opus 对抗验证抓获双改判 ask-后-auto
泄漏与 AOE 闪化解路径缺挂起守卫, 同批修正。执行记录: 同上。

## v13-audit4 — 第四轮全量审计 (16 修复)

用户指令「检查已实现技能与所有牌的逻辑」: 12 域并行清查 (每发现必附
实跑复现) + 逐条对抗驳斥 + 覆盖度审查 (53 技能 + 33 牌型零漏审):
确证 16 (高2/中9/低5), 驳倒 2, 归档已知偏差 3; 同批修复全部 16 条 +
3 项连带收口, 回归 20 例; H1 守恒 fuzz 1200 种子零失败。明细:
`docs/audit/2026-07-19-audit4-skills-cards.md`。

## v13-张角二修+UI — 实测 7 项

鬼道独立面板 (不再复用鬼才面板致观感雷同); 鬼道可用手牌+装备黑牌
(官方 spec "黑色牌"非"黑色手牌" — 即"判定牌不生效时不问"的真因,
AI/auto 不自动弃装备); 决策提示与活动信息栏花色带色形状 (colorizeSuits
后处理英文花色词); 日志不强制跳底 (保留玩家上翻位置); AI 节奏放慢
(1300→1700ms)。评审收口 5 处低打磨 (guidao holder-scan canPay 对齐等),
opus 对抗六类实跑全 SAFE。执行记录: 同上。

## v13-张角三修 — 官方"替换≠代替"落地 + 延时锦囊无懈时机

机制真因: 官方术语表 (glossary__gamecard) **替换** = 你获得原判定牌
(鬼道), **代替** = 原判定牌进弃牌堆 (鬼才) — 引擎三条鬼道路径统一
settleReplacedOriginal(gainOriginal), 鬼才仍弃置; 决策面板浅底黑花色
改深墨色 (作用域覆盖, 不影响棋盘); 延时锦囊判定按官方"无使用者"语义
wuxie ctx actor:null — 放置者判定前同样被询问无懈。opus 对抗验证 E(b)
抓获 wuxiePolicy='always' 的 AI 放置者自耗无懈取消己方延时锦囊的真
回归 → 立场过滤移到 always 早退之前, 同批修正; 删 judge-area 死字段
placedBy。执行记录: 同上。

## v14 — 工程清账 / 多目标杀架构 / AI 博弈深化 / 内容决策门

O 工程清账 (harness 统一样板 + 测试快·全分档 + 卫生清零 + gid 收案);
P **多目标杀架构** (目标队列驱动: 流离 → 武将技锁定 → 装备技 → 逐席结算
四阶段, 方天画戟额外目标与流离转移双双销账; 官方判例"转移给既有目标 B 后
B 被连续结算两次"逐字落地); Q AI 博弈深化 (无懈全类期望值建模 + 内奸带
量化门禁的装忠/拆家/收割序 + 突袭真 ask); R 决策门 — 用户一次性拍板:
KOF 不立项留占位、战报回放/移动端/国际化收案、6 人以上大场收案 (5 人封顶),
R-A 收尾批 (推断提示徽章/座席环压缩/死钮清理), R-B 蛊惑 (于吉, 风包现行版)
虚拟声明牌层 + 质疑链 + 缠怨。守恒 fuzz 1200 种子档在 P4 抓获两条预存缺陷
(青囊座席二元回退 / harness zone 名笔误) 并连带收口。执行记录:
`docs/plans/2026-07-31-sanguosha-v14-roadmap.md`。

## v15 — 三包 48 技接入 + 蛊惑收尾 + 清账收官 (已收官)

五个阶段一次成型, 每批独立 PR、推送前核对上游 PR 状态:

**S 蛊惑收尾** — 响应窗口的"打出流程"接入八个窗口 (此前蛊惑只覆盖使用
流程), 声明菜单补齐 16/16 牌型, AI 全型声明启发 + 自洽性测试。明细:
`docs/audit/2026-08-05-guhuo-spec-gaps.md`。

**T 火包 8 将 13 技** + **拼点框架** (全新机制: 双方扣置手牌比点数, 官方
顺序"赢/没赢后效果 → 处理区拼点牌入弃牌堆", 效果挂起时留账由选择排空后
flush)。评审收口 22 钉, 其中 H1 是"猛进早退吞掉闪避分支"—— 教训写进架构
注释: **绝不能用 `if (game.pendingChoice)` 判断"我这一跳是否挂起了"**,
那个槽位可能已被无关窗口占用 (张角雷击经 onShanUsed 抢占), hook 必须返回
自己的挂起信号。明细: `docs/audit/2026-08-05-fire-pack-spec.md`。

**U 林包 8 将 18 技** — 祸首/巨象的南蛮生命周期、乱武逐席链、帷幕按
"目标合法性"在建队列时过滤。opus 对抗复核实测 4 条 P0 / 3 条 P1, 修 6 条、
如实归档 1 条 (双闪同步循环无挂起停机 —— 正确改法与 v10 V3 架构守护冲突,
留待独立批)。明细: `docs/audit/2026-08-06-lin-pack-spec.md`。

**V 山包 7 将 15 技** (+ 凿险授予的派生技急袭) — 觉醒技共用骨架、武将牌上
置牌区"田"(入守恒 census)、"于回合外失去牌后"时机 (带重入闸)、额外回合
机制。**左慈化身经路线图明列的独立成本评估门后推迟**: 化身需要动态技能
三态层 (138 处 `hasSkill(` 无单一入口)、性别·势力覆写单点 (11 处直读、
零单点)、场外武将牌堆三条横切基建; 新生脱离化身零可观测效果 → 一并推迟,
fixture 显式标 deferred 并写明理由, 其余 7 将不受连累。明细:
`docs/audit/2026-08-06-shan-pack-spec.md`。

**W 清账收官 + 第五轮全量合规审计** — 两件事:

*W1 散账清零* (六项"做/收案"二态归零, 不许继续无声漂移): AI 反间挂起裸返
无 `ok` 字段的畸形结果 (v14 P4 唯一显式滚动未回收缺陷) 收口; 947 行 41 段
重复 if/else 的提示面板改**表驱动注册表**; 克己按官方"你可以"补偏好闸;
randomSuit 的 `Math.random` 兜底改**确定性**推导 (种子可复现是守恒 fuzz 的
前提); M9 防御性回退分支与 jsdom 账本**收案** —— 后者顺带更正了路线图草稿的
一句不实前提 (Playwright 是 v13 的一次性验证, 仓库里没有任何脚本入库;
真正成立的理由是零依赖红线 + 23 个 fake-DOM 文件 / 210 个执行用例)。

*W2 第五轮审计*: 10 域并行清查 → opus 逐条对抗验证 → 完备性批判,
**27 个 agent、16 条候选 → 确证 11 / 驳倒 5**。本批修复 7 条, 其中三条**高危**
且两条打在自己身上: 屯田的「距离 -X」**从未接入** (而 V 阶段那条测试挑了基础
距离为 1 的席位对, 空跑通过); 断肠/行殇 的先后**在 V 阶段判反了**还被写成
"官方判例" (真正的官方规则是同一时机从当前回合角色起按逆时针依次, 于是顺序
取决于这是谁的回合 → 改为逐席派发); `aoe-sha-response` 窗口 UI 里**一个面板
都没有**, 于吉被南蛮指定后**整局卡死**; 暴虐在**伤害致死时静默失效**。
另有一条驳回同样打在本轮自己的修复上并已还原 (据（风）曹仁改小据守 ——
（风）曹仁是「据守 + 解围」的双技重做卡), 由此确立口径:
**换版本的前提是整张武将牌的技能集一致, 逐条挑版本无效**。
确证但本批未修的 8 条逐条留账 (复现/官方出处/修法与对抗验证点名的坑俱全)。

审计本身是一次性的, 真正留下来的是三条**常驻不变量** (pendingChoice kind ×
UI 渲染路径 / 技能 metadata hook 名 × 实际注册 / 39 牌型 × 使用出口) 与
**固化入库的守恒 soak** (v14 P4 那套一次性 scratch 终于常驻; 本批实跑
1200 种子 / 65253 步零失败, 当场抓到上述两条)。
账本: `docs/audit/2026-08-06-w-ledger.md`。

**X 留账清偿 (2026-08-07)** — W 批"确证未修"的 F8-F14 八项一次性清零,
对抗验证点名的坑逐条按账处理: 借刀闸口与点火**同点收口**到转化感知单点
(半拉子教训的正解), 丈八虚拟杀回滚走弃牌堆取回 (守恒钉); 猛进/贯石斧按
"武将技先于装备技"重排并成立**连续双挂起**链; 闪电移动过帷幕窄谓词;
银月枪补"使用"半面 (借刀/挑衅逼出的黑杀); 妄尊移到准备阶段唯一收束点
(观星等回合角色技能之后、不被挂起吞掉); 铁骑锁定前移到雌雄之前并随快照
携带; AOE 逐目标无懈窗口展示目标补齐。全档 206 文件绿, soak 600 种子
加档零失败。明细: `docs/audit/2026-08-07-x-ledger-clear.md`。

## v16 — Y 响应链可挂起化（2026-09-07）

杀（含双闪）、AOE、决斗、濒死、无懈五条响应链采用统一的数据帧协议，
逐张消费后可以挂起，内层结算完成后续跑。铁索传导也登记嵌套位置，避免
雷击引出的救援被第二张闪越过。Y-G1 选择 (b)，保留禁止手写响应窗口的
架构守护。AI 蛊惑响应八窗口同批交付，原生响应优先，验假自动恢复原流程。

四条留账清零：队列候选刷新、帷幕实际花色、武圣红武器原槽回滚、闪电
全环失败日志按实因记录。Y5 通用钩子链早退不天然适用逐牌协议，按条件范围
显式滚动 AC4。新增 45 例（引擎 37 / UI 4 / 协议守护 4），全档 206→209
文件绿，600 种子守恒通过，三基准门槛不变。当时后续 Z/AA/AB/AC 尚未执行；Z 已由下述批次交付。
明细：`docs/audit/2026-09-07-v16-y-response-protocol.md`。

## v16 — Z 逐字源残余回收（2026-09-07）

既有目录内的单骑、庸肆、伪帝三技接入，71 将保持不变，已接入技能由
101 增至 104（123 个挂将唯一 ID 中仍有 20 个未接入，另含派生技急袭）。
伪帝覆盖九种主公技与若愚两个官方授予顺序；庸肆支持手牌/装备成本，衔接
枭姬、固政和正常手牌上限。巧变补上跳过出牌阶段后的装备/判定区移牌，
四段选择绑定窗口身份，合法性与暂停续跑均有回归，旧功能缺口记录已清除。

AI 在轻伤、无弃牌压力、已有响应足以覆盖公开估计来杀时保留最后一桃，
受到攻击压力时仍可回血。拼点改为双方扣置后、亮牌前提交失牌效果，避免
连营摸桃晚于驱虎救援；补全暂停、认领、克隆隔离与终局清算。

独立 Codex 代理复核及全档促成庸肆保桃/固政、伪帝黄天和制霸分别计次、
黄天 AI 暗身份读取、暴虐来源势力快照等交互修复。新增 85 例（SP 28 / 巧变
29 / AI 与拼点 25 / 独立回归 3）。旧守恒 soak 曾误读不存在的 total 字段，
本批改为真实 ID 集合与重复占区检查，并加入两名 SP 武将及庸肆决策支持。
此前 soak 的计数结论按本批审计更正，其他逐例守恒断言仍有效。

Z-G1 判定木牛流马属于界限突破的宝物及新牌区扩展，按默认边界登记不做，
保留 39 牌型。全档 215 文件、600 种子 / 31,334 步守恒及三基准门禁通过。
AA/AB/AC 仍按路线图安排。最终测试与三基准实测见
`docs/audit/2026-09-07-v16-z-completion.md`。


## v16 — AA 动态技能与场外武将牌（2026-09-08）

完成技能拥有/有效性的唯一裁决、156处旧查询收口、30处当前性别/势力读口和71 ID独立武将牌池；亮将后/回合开始/结束后三边界支持暂停与JSON续跑，UI明确区分动态有效/无效技能。对抗复核修复已有技能持续效果、无双整牌需求、不屈上限公式与额外回合返回锚点。

AA-G2采用全量运行态属性迁移；AA-G1按基建后的无候选开局反例与逐点调度缺口继续推迟化身/新生，图鉴不标完成。已接入技能仍104个，保留完整71将池。明细：`docs/audit/2026-09-08-v16-aa-completion.md`。

## v16 — AB 八神将（2026-09-08）

用户明确开启AB-G1的小粒度内容门，按风火林山LE001–LE008整卡加入八神将，20原生技能与派生极略。包括神将势力选择、七星场外实体区、同花色多材料龙魂、暴怒／忍／梦魇、天气、无前临时效果、死亡判定和连破额外回合。判定和多段技能改用可暂停数据帧；原有71将名称及当时技能集保留。

AB收尾239测试文件、214新增执行用例与600种子神将加档通过，合并PR #199后Pages部署成功。该批时点为79将／148挂将技能槽／143原生唯一ID／125已实现ID／20原生TODO。后续AC发现并补列新风周泰奋激TODO，历史统计不回写。明细：`docs/audit/2026-09-08-v16-ab-completion.md`。

## v16 — AC 第六轮审计与收官（2026-09-08）

完整十域查源、真实引擎对抗、独立重跑与交叉完整性复核，重点审查Y数据帧、AA动态资格和AB嵌套判定。修复JSON后函数丢失、涅槃重复判定牌、死亡后锦囊发牌／回血、终局在途牌消失、无懈行动顺序及自身响应、挑衅原生转换、狂骨时机、强袭来源死亡、放权已付效果、缔盟完整支付与同时失牌、悲歌装备成本、琴音无效选项、固政实体回绑等问题。官方版本不一致的旧测试按逐字来源纠正；被驳倒的假设和既有简化明确留在审计报告。

三条新常驻不变量加上既有三条，检查48个技能ID的62个直接hook路径、计算出的当前README统计，以及五条响应链强制通过统一暂停／终局出口。AI对T/U/V的46原生技能及急袭统一实测合法机会、使用和局部收益，另列Z与神将；修复转换估值、田区急袭、巧变／放权默认从不使用，以及天义花掉唯一杀的浪费。原三个基准阵容、种子与门槛保留。

英魂／悲歌被动弃牌改由本人选择手牌或装备，并使用可JSON恢复的支付帧。六个旧面板已接入的事实经实际行为重验，陈旧缺口注释清除；开局无回合时不再错误标记电脑行动。C25通用多hook交错调度按当前79张原生卡实际无冲突的新盘点收案；AC-G1第二层浏览器测试依新覆盖度量和实际缺陷层次再次收案，零npm依赖保持。化身／新生明确滚动下一独立内容／调度批，不宣称已实现。

C1–C30逐项归为已交付、已收案或显式滚动。新风周泰补回官方整卡技能奋激，标为TODO；现为79将／149技能槽／144原生唯一ID／125已实现ID／21原生TODO。完整矩阵、局限重开条件、实测与远端验收见`docs/audit/2026-09-08-v16-ac-completion.md`。

## v16 — 最终发布与移交（2026-09-08）

AC实现#200通过PR CI后合并；首次main Pages在旧黄天UI测试失败。根与独立复核者使用固定种子重现随机AI将主公打至0体力的无效测试前提；补救#201改用真实选将、空牌轮转及存活资格断言，保留完整按钮交牌操作，没有更改生产规则。

最终main `03cdc8d624c5207482436dcdc8fbfd40dc44b09a` 的251测试文件全绿，Pages于08:37 UTC发布成功，线上神将选择、装备、出杀、人工出闪和回合恢复通过。09:13 UTC再次核验远端仅main，原两条工作分支已清理。加档、AI口径、运行链接与剩余边界见[v16最终发布收尾](audit/2026-09-08-v16-release-closeout.md)。

同日形成[v17路线图](plans/2026-09-08-sanguosha-v17-roadmap.md)：下一阶段AD补同一时机自由顺序与可恢复调度，后续安排移动事实、既有交互、化身双门及审计。此项是规划交付，AD–AH尚未实施。
