      ...
      export function installStandardSkillHandlers(skillRegistry, deps) {
        ...
        var triggerGuidaoJudgementBeforeResolve = deps.triggerGuidaoJudgementBeforeResolve;
        ...
        var triggerQiangxiActiveSkill = deps.triggerQiangxiActiveSkill;
        ...
        onTurnEnd: function (context) {
          return triggerJushouTurnEnd(context.game, context.actor);
        }
      });
        ...
        onNeedResponse: function (context) {
          var source = context.game[context.actor];
          var target = context.game[context.targetActor];
          if (!source || !target || !hasSkill(source, 'liegong') || context.responseType !== 'shan') return null;
          if ((target.hand || []).length >= source.hp || (target.hand || []).length <= 1) {
            context.responseLocked = true;
            log(context.game, actorName(context.game, context.actor) + '的【烈弓】生效，目标不能使用【闪】。');
        ...
        SkillRuntime.registerSkill(skillRegistry, 'hongyan', {
        // 红颜由判定域 effectiveJudgementCard 统一改写黑桃判定牌；注册保留审计锚点。
        onPassiveReady: function () { return null; }
      });
        SkillRuntime.registerSkill(skillRegistry, 'leiji', {
        // 雷击由 consumeResponse 在打出【闪】后触发；注册保留技能域锚点。
        onPassiveReady: function () { return null; }
      });
        SkillRuntime.registerSkill(skillRegistry, 'buqu', {
        // 不屈由濒死域 processDyingNext 在死亡前触发；注册保留技能域锚点。
        onPassiveReady: function () { return null; }
      });
        SkillRuntime.registerSkill(skillRegistry, 'mengjin', {
        // 猛进由【杀】被【闪】抵消后的收尾分支触发；注册保留技能域锚点。
        onPassiveReady: function () { return null; }
      });
