      ...
      addHeroPack('standard', [
        ...
        // v11 C8 (批次 32): 官方 spec 缓存 (gid 215) — 标准版袁术
        { id: 'yuanshu', name: '袁术', camp: '群', gender: 'male', title: '冢中枯骨', maxHp: 4, quote: '大汉天下，已半入吾手！', skills: [
          { id: 'wangzun', name: '妄尊', desc: '主公的准备阶段开始时，你可以摸一张牌，该主公本回合手牌上限 -1。' },
          { id: 'tongji', name: '同疾', desc: '锁定技：手牌数大于体力值时，攻击范围内含你的角色使用【杀】不能指定除你以外的目标。' }
        ] }
      ]);
      ...
      var SKILL_METADATA = {
        ...
        // 神速/红颜曾在本批按"宁缺毋滥"撤出伪实现, 已于 v12 G2 补齐所需
        ...
        yinghun:  { trigger: 'preparePhase',       frequency: 'oncePerTurn',     optional: true,  mandatory: false, cost: { type: 'none' },                     hooks: ['onPreparePhase'] },
        haoshi:   { trigger: 'drawPhase',          frequency: 'oncePerTurn',     optional: true,  mandatory: false, cost: { type: 'giveHand',   count: 'any' }, hooks: ['onDrawPhase'] },
        dimeng:   { trigger: 'playPhase',          frequency: 'oncePerTurn',     optional: true,  mandatory: false, cost: { type: 'discardOwn', count: 'any' }, hooks: ['onActiveSkill'] },
        huoshou:  { trigger: 'passive',            frequency: 'passiveAlways',   optional: false, mandatory: true,  cost: { type: 'none' },                     hooks: ['skillEnabled'] },
        zaiqi:    { trigger: 'drawPhase',          frequency: 'oncePerTurn',     optional: true,  mandatory: false, cost: { type: 'reduceDraw', count: 2 },     hooks: ['onDrawPhase'] },
        juxiang:  { trigger: 'passive',            frequency: 'passiveAlways',   optional: false, mandatory: true,  cost: { type: 'none' },                     hooks: ['skillEnabled'] },
        lieren:   { trigger: 'damageAfter',        frequency: 'unlimited',       optional: true,  mandatory: false, cost: { type: 'rankCompare' },              hooks: ['onShaDamageDealt'] },
        wansha:   { trigger: 'passive',            frequency: 'passiveAlways',   optional: false, mandatory: true,  cost: { type: 'none' },                     hooks: ['skillEnabled'] },
        luanwu:   { trigger: 'playPhase',          frequency: 'oncePerGame',     optional: true,  mandatory: false, cost: { type: 'none' },                     hooks: ['onActiveSkill'] },
        weimu:    { trigger: 'targetValidation',   frequency: 'passiveAlways',   optional: false, mandatory: true,  cost: { type: 'none' },                     hooks: ['skillEnabled'] },
        jiuchi:   { trigger: 'cardConvert',        frequency: 'unlimited',       optional: true,  mandatory: false, cost: { type: 'playHand',   count: 1 },     hooks: ['onCardAs'] },
        roulin:   { trigger: 'passive',            frequency: 'passiveAlways',   optional: false, mandatory: true,  cost: { type: 'none' },                     hooks: ['skillEnabled'] },
