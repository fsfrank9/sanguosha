---
source: https://gltjk.com/sanguosha/rules/card/scroll.html
title: "锦囊牌 | 三国杀官方规则集 3.0"
content_sha256: 424be6131a5d0b1a9ba4045931e2c6904c6703d9ac04b90509de7f4905d3c70a
---
...
>
注意：使用【借刀杀人】的过程中须进行两次使用【杀】的合法性检测：第一次是在你选择An使用【杀】的目标Bn时（此时【杀】并未置入处理区，Bn只是被选择为将要使用的【杀】的目标），因此你不能对攻击范围内没有使用【杀】的合法目标的角色使用【借刀杀人】；第二次是在An选择是否对Bn使用【杀】时，须检测Bn是否为An使用【杀】的合法目标。
...
- 使用时机：出牌阶段。
- 使用目标：包括你在内的一名角色。
- 作用效果：目标角色摸X张牌（若全场对方角色数不大于己方，X为2；若全场对方角色数大于己方，X为3）。
...
◆【无懈可击】有两种，此【无懈可击】即普通【无懈可击】，此外还有国【无懈可击】（国-标）。
...
- 使用时机：出牌阶段。
- 使用目标：所有角色。
- 执行动作：1.你在选择目标的同时选择结算的方向。2.当此牌指定目标后，你亮出牌堆顶的X张牌（X为目标数）。
- 作用效果：目标角色获得这些牌中（剩余）的一张牌。
...
例如：全场只有左慈和A两名角色，左慈的判定区里有一张黑色的【闪电】，A的判定区里有一张红色的【闪电】。左慈在回合开始时发动【化身②】拥有【帷幕】，在判定阶段进行【闪电】的判定，结果不为黑桃2~9，由于左慈和A都不是此【闪电】的合法目标，将之置入左慈的判定区。
...
- 使用时机：出牌阶段。
- 使用目标：你和与你势力相同的所有其他角色。
- 作用效果：目标角色摸两张牌，若如此做，当此【以逸待劳】对所有目标的使用结算结束时，该角色弃置两张牌。
...
- 使用时机：出牌阶段。
- 使用目标：一名其他角色。
- 作用效果：你选择一项：1.观看目标角色的手牌；2.观看目标角色的一张暗置的武将牌。
